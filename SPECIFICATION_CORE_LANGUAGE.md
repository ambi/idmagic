...
> **この文書の読み方**: 本書も section-addressable なリファレンスである。通し読みではなく、§3 冒頭の「変更のきっかけ→見直す節」表を索引に該当節だけを引く。
...
SCL のセクションは実装構造へ 1:1 に写る。`models` は仕様バインディングを経て言語の型（Zod・Pydantic・Go 構造体）になり、bounded context はディレクトリになる。仕様の構造・ディレクトリの構造・データ型の構造を一致させることが、AI と人間の双方にとっての可読性と局所化を生む（[REGENERATIVE_ARCHITECTURE.md](REGENERATIVE_ARCHITECTURE.md) §3.6）。
...
`scope` に列挙する。この表は、きっかけからコードではなく仕様の該当節へ最短で
到達するための、触るべき仕様箇所の即時特定索引でもある。
...
複数 SCL ドキュメントへ分割した後は、内部モデルを直接共有せず、`publishes` と `depends_on` で公開言語を明示する。マップキーはコンテキスト名（各ドキュメントの `system` に対応）で、各エントリのプロパティ（`path`・`description`・`publishes`・`depends_on`）と `Dependency`（`uses`・`via`・`reason`）は §2.2 が定める。`publishes` の既定は空（全面非公開）、`depends_on.<ctx>.uses` の各要素は上流の `publishes` に含まれていなければならない。
...
`invariants.assuming` / `invariants.always` / `invariants.never` / `invariants.eventually`、`states.transitions.guard`、`permissions.allow_when` / `permissions.deny_when` に書ける式。式はすべて平坦な**文字列式**として記述する。

### 5.1 文字列式: CEL (Common Expression Language)

文字列式は [Common Expression Language (CEL)](https://github.com/google/cel-spec) に従う。型規則・演算子優先順位・null 意味論・評価順は CEL 仕様が規定する。SCL はその上に変数バインディング（§5.3）を追加するのみである。
...
| 関数・マクロ | `size(x)`、`has(x.y)`、`.exists(x, expr)`、`.all(x, expr)`、`.exists_one(x, expr)`、`.filter(x, expr)`、`.map(x, expr)` |
