...
### 6.1 SCL element reference

SCL の外側にある Architecture、実装 binding、検証宣言、実行証跡は、規範要素を
context-qualified な構造化値で参照する。参照を SCL 文書内へ逆向きに埋め込まず、規範文の
正本は引き続きそれを実現・検証する所有要素に置く。
...
外部標準 requirement だけは、standard の map key と既存 requirement `id` の組を用いる。

```yaml
context: System
kind: standard_requirement
standard: WCAG22
requirement: WCAG22-KEYBOARD
```

参照可能な `kind` と解決先は次のとおり。

| `kind` | 解決先 |
| --- | --- |
| `standard_requirement` | `standards.<standard>.requirements[id == requirement]` |
| `model` | `models.<element>` |
| `interface` | `interfaces.<element>` |
| `state` | `states.<element>` |
| `authorization_resource` | `authorization.resources.<element>` |
| `authorization_principal` | `authorization.principals.<element>` |
| `authorization_policy` | `authorization.policies.<element>` |
| `objective` | `objectives.<element>` |
| `scenario` | `scenarios.<element>` |
| `flow` | `flows.<element>` |

`context` と `kind` は常に MUST。`standard_requirement` 以外は `element` が MUST で
`standard` / `requirement` を持ってはならない。`standard_requirement` は `standard` と
`requirement` が MUST で `element` を持ってはならない。未知 field は拒否する。

workspace では `context` を root `context_map` の map keyへ完全一致で解決し、map entry の
`path` が指す文書の `context` も同じ値でなければならない。暗黙の current context、別 context
への fallback、大文字小文字を無視する解決は行わない。context map を持たない単一 SCL 文書を
表示するときだけ、文書の `context`、それがなければ `system` を表示上の context とする。

通常要素の identity は各 section の map key、standard requirement の identity は standard map
key と requirement `id` の組である。同一 standard 内の requirement `id` は一意でなければならない。
配列 index、YAML 行番号、HTML slug を identity にしない。個々の model constraint、interface
`requires` / `ensures`、state transition、scenario step のような匿名配列要素は、その所有要素を
参照する。

canonical text は通常要素を `<context>/<kind>/<element>`、standard requirement を
`<context>/standard_requirement/<standard>/<requirement>` と表す。各 path segment 内の `~` は
`~0`、`/` は `~1` に escape する。HTML anchor を含む派生識別子は、この構造化値から生成し、
別の省略名や位置情報から組み立ててはならない。

`glossary`、`annotations`、`context_map`、派生 artifact は verification target に含めない。
element rename は参照 identity の破壊的変更である。
...
[ADR-112](decisions/ADR-112-scl-flow-view-registry-and-cel-binding-reference.md) に、規範要素を
独立した保証台帳へ複製せず直接参照する判断は
[ADR-114](decisions/ADR-114-direct-context-qualified-scl-element-references.md) に記録する。
