...
1. `spec_version` dispatcher で version schema を一つ選ぶ。
...
同じ文書に SCL 2.0 / 3.0 field を混在させてはならない。shape と意味検証は別 finding として
報告できるが、いずれか一つでも error があれば文書は invalid である。
...
## 8 versioning と SCL 2.0 移行 bridge

### 8.1 `spec_version` dispatcher

`spec_version` は schema 自体の version であり、製品 version ではない。validator は明示的 dispatcher を
持ち、`"2.0"` には凍結済み `scl-v2.schema.json`、`"3.0"` には
`scl-v3.schema.json` を適用する。欠落、未知 version、version と field の混在を拒否する。

SCL 2.0 dispatcher は wi-208〜wi-211 の workspace 移行だけを成立させる内部 bridge である。
public compatibility promise ではなく、wi-212 で v2 schema と dispatcher を削除する。

### 8.2 SCL 2.0 から 3.0 への所有先

| SCL 2.0 | SCL 3.0 |
| --- | --- |
| field 形式・範囲 invariant | field `constraints` |
| model 内整合 invariant | model `constraints` |
| 操作の前提 / 事後 invariant | interface `requires` / `ensures` |
| state invariant | transition `guard` / `effect`、`terminal` |
| `permissions` | `authorization` + interface `access` |
| SLO objective | 新 `objectives` |
| retention / TTL / security objective | model lifecycle、constraint、interface、state、scenario、ADR |
| `user_experience.transitions` | `flows` |
| screen / route / UX requirement | scenario、standard、objective、実装または ADR の所有先 |
| scenario `steps` / Cockburn 混在 | `actor` + `main_success` + `extensions` |
| `relates_to` / `protects` | 意味 field、interface `access`、standard requirement `refs` |

移行は名前変更ではなく、各規則の所有者を判定して移す。SCL 3.0 schema は廃止 field を一切受理しない。
