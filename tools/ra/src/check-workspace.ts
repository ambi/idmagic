...
if (args.has('--help') || args.has('-h')) {
  process.stdout.write('Usage: ra check [--work-items] [--ids] [--architecture] [--requirements]\n')
  process.exit(0)
}
const valid = new Set(['--work-items', '--ids', '--architecture', '--requirements'])
...
if ((all || args.has('--requirements')) && config.requirements.length > 0) {
  await runTool(['check/src/check-requirements.ts', ...config.requirements.map(rootPath)])
}

if ((all || args.has('--architecture')) && config.architectureDocs.length > 0) {
  await runTool([
    'check/src/main.ts',
    '--schema=architecture-doc',
    ...config.architectureDocs.map(rootPath),
  ])
}
