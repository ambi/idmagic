...
const operations: Array<{ name: string; method: string; path: string; deprecated: boolean }> = []
...
const entries = operations
  .map(
    ({ name, method, path, deprecated }) =>
      `\t${quote(name)}: {Method: ${quote(method)}, Path: ${quote(path)}, Deprecated: ${deprecated}},`,
  )
  .join('\n')
