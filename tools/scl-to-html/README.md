...
```bash
bun scl-to-html \
  --scl       ../path/to/spec/scl.yaml \
  --title     "app-name" \
  --out       out/app-name.html
```
...
bun run scl-render:workspace        # Generate discovered SCL HTML artifacts
```
