...
type TabKey = 'scl' | 'architecture' | 'decisions' | 'work-items'
...
const availableTabs = (site: SiteInput): TabKey[] => {
  ...
  if (site.architecture) tabs.push('architecture')
...
export const renderPage = (site: SiteInput): string => {
  ...
  const bodies: Record<TabKey, string> = {
    ...
    architecture: renderArchitectureTab(site.architecture),
  ...
  const tocItems: Record<TabKey, Array<{ id: string; label: string }>> = {
    scl: sclTocItems(scl),
    architecture: architectureTocItems(site.architecture),
