...
const taskSummary = (tasks: string | undefined): { done: number; total: number } | null => {
  if (!tasks) return null
  const matches = [...tasks.matchAll(/^\s*-\s+\[([ xX])\]\s+T\d{3}\b/gm)]
...
const renderWorkItem = (wi: WorkItem): string => {
  ...
  const tasks = taskSummary(wi.tasks)
  const meta = [
    wi.status ? badge(wi.status, statusKind) : '',
    wi.risk ? badge(`risk: ${wi.risk}`, riskKind) : '',
    tasks ? chip(`tasks: ${tasks.done}/${tasks.total}`, 'hint') : '',
  ...
  if (wi.plan) {
    blocks.push(
      `<details class="change-block" open><summary>Plan</summary>${renderProse(wi.plan)}</details>`,
    )
  }
  if (wi.tasks) {
    const summary = taskSummary(wi.tasks)
    const label = summary ? `Tasks (${summary.done}/${summary.total})` : 'Tasks'
    blocks.push(
      `<details class="change-block" open><summary>${esc(label)}</summary>${renderProse(wi.tasks)}</details>`,
    )
  }
  ...
  const index = sorted
    .map((c) => {
      ...
      return `<a class="ch-index-row" href="#${esc(slug(c.id))}">
        ...
        ${tasks ? chip(`${tasks.done}/${tasks.total} tasks`, 'hint') : ''}
