...
describe('verifyWorkItemDependencies', () => {
  it('accepts an acyclic graph including completed prerequisites', () => {
    expect(
      verifyWorkItemDependencies([record('wi-1-foundation'), record('wi-2-feature', ['wi-1-foundation'])]),
    ).toEqual([])
  })
  ...
  it('reports an unknown prerequisite at the dependency field', () => {
    ...
    expect(findings[0]).toMatchObject({ line: 6, message: expect.stringContaining('unknown work item') })
  })
