...
function markdownRenderer(
  ...
  md.renderer.rules.text = (tokens, index, options, env, self) => {
    ...
    const match = content.match(/^(Given|When|Then|And|But)\s+([\s\S]+)$/)
