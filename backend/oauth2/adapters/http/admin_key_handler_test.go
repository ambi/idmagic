...
func newKeyAdminServer(t *testing.T, actor *userdomain.User) (*echo.Echo, *signingcrypto.InMemoryKeyStore, *[]spec.DomainEvent) {
	t.Helper()
...
func keyAdminUser(sub, tenantID string, roles []string) *userdomain.User {
	now := time.Now().UTC()
