...
func TestValidateHappyAndFailure(t *testing.T) {
	...
	validBinding := agentdomain.AgentCredentialBinding{AgentID: "agent_1", ClientID: "demo", CreatedAt: now}
	badBinding := agentdomain.AgentCredentialBinding{CreatedAt: now}

	validGroup := groupdomain.Group{ID: "group_1", TenantID: tenancydomain.DefaultTenantID, Name: "eng", CreatedAt: now, UpdatedAt: now}
	badGroup := validGroup
	badGroup.Name = ""

	validMember := groupdomain.GroupMember{GroupID: "group_1", UserID: "user_1", CreatedAt: now}
	badMember := groupdomain.GroupMember{UserID: "user_1", CreatedAt: now}
...
func TestNewIDsAreUUIDs(t *testing.T) {
	agentID, err := agentdomain.NewAgentID()
	if err != nil {
	...
	groupID, err := groupdomain.NewGroupID()
	if err != nil {
...
func TestAgentIsActive(t *testing.T) {
	now := time.Now().UTC()
	active := agentdomain.Agent{Status: idmdomain.AgentStatusActive}
	if !active.IsActive() {
		t.Fatal("active agent must be active")
	}
	disabledStatus := agentdomain.Agent{Status: idmdomain.AgentStatusDisabled}
	if disabledStatus.IsActive() {
		t.Fatal("disabled status must not be active")
	}
	withDisabledAt := agentdomain.Agent{Status: idmdomain.AgentStatusActive, DisabledAt: &now}
	if withDisabledAt.IsActive() {
		t.Fatal("disabled_at set must not be active")
	}
	withKilledAt := agentdomain.Agent{Status: idmdomain.AgentStatusActive, KilledAt: &now}
