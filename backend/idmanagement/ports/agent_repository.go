...
type AgentRepository interface {
	ListByTenant(ctx context.Context, tenantID string) ([]*idmdomain.Agent, error)
	FindByID(ctx context.Context, tenantID, id string) (*idmdomain.Agent, error)
	Save(ctx context.Context, agent *idmdomain.Agent) error
	Delete(ctx context.Context, tenantID, id string) error

	ListBindings(ctx context.Context, tenantID, agentID string) ([]*idmdomain.AgentCredentialBinding, error)
	// AddBinding は束縛を追加し、新規追加なら true を返す。既に束縛済みなら false を
	// 返し no-op とする (冪等)。
	AddBinding(ctx context.Context, binding *idmdomain.AgentCredentialBinding) (bool, error)
	...
	FindByClientID(ctx context.Context, tenantID, clientID string) (*idmdomain.Agent, error)
}
