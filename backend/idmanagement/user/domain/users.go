...
func (u User) IsSoftDeleted() bool {
	return u.Lifecycle.EffectiveStatus() == idmdomain.UserStatusPendingDeletion
}
...
func (l UserLifecycle) EffectiveStatus() idmdomain.UserStatus {
	if l.Status == "" {
...
func (v AttributeValue) Validate() error {
	...
	set := 0
	matches := false
	check := func(present bool, t idmdomain.AttributeType) {
		if present {
			set++
			if v.Type == t {
				matches = true
	...
	check(v.String != nil, idmdomain.AttributeTypeString)
	check(v.Number != nil, idmdomain.AttributeTypeNumber)
	check(v.Boolean != nil, idmdomain.AttributeTypeBoolean)
	check(v.Date != nil, idmdomain.AttributeTypeDate)
	check(v.StringArray != nil, idmdomain.AttributeTypeStringArray)
	if set != 1 || !matches {
...
type UserAttributeDef struct {
	...
	Visibility     idmdomain.AttrVisibility `json:"visibility"`
