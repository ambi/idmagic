...
func effectiveRuleDefs(ctx context.Context, deps DynamicGroupDeps, tenantID string) ([]userdomain.UserAttributeDef, error) {
...
func UpdateDynamicGroupRule(ctx context.Context, deps DynamicGroupDeps, actorUserID, groupID, expression string, now time.Time) (*groupdomain.DynamicGroupRule, error) {
	...
	version := int64(1)
	createdAt := idmusecases.NormalizedNow(now)
	enabled := false
	if existing != nil {
	...
	rule := &groupdomain.DynamicGroupRule{GroupID: groupID, TenantID: tenantID, Expression: expression, Enabled: enabled, Version: version, ReferencedAttributes: compiled.References(), CreatedAt: createdAt, UpdatedAt: idmusecases.NormalizedNow(now)}
	...
	if err := idmusecases.AdminEmit(deps.Emit, &idmdomain.DynamicGroupRuleUpdated{At: idmusecases.NormalizedNow(now), TenantID: tenantID, ActorUserID: actorUserID, GroupID: groupID, RuleVersion: rule.Version}); err != nil {
		return nil, err
	}
	if rule.Enabled {
		err = scheduleDynamicGroupReconcile(ctx, deps, rule, idmusecases.NormalizedNow(now))
	}
	return rule, err
}
...
func SetDynamicGroupRuleEnabled(ctx context.Context, deps DynamicGroupDeps, actorUserID, groupID string, enabled bool, now time.Time) (*groupdomain.DynamicGroupRule, error) {
	...
	rule.Version++
	rule.UpdatedAt = idmusecases.NormalizedNow(now)
	if err := deps.GroupRepo.SaveDynamicRule(ctx, rule); err != nil {
		return nil, err
	}
	var event spec.DomainEvent = &idmdomain.DynamicGroupRuleDisabled{At: idmusecases.NormalizedNow(now), TenantID: tenantID, ActorUserID: actorUserID, GroupID: groupID, RuleVersion: rule.Version}
	if enabled {
		event = &idmdomain.DynamicGroupRuleEnabled{At: idmusecases.NormalizedNow(now), TenantID: tenantID, ActorUserID: actorUserID, GroupID: groupID, RuleVersion: rule.Version}
	}
	if err := idmusecases.AdminEmit(deps.Emit, event); err != nil {
		return nil, err
	}
	if enabled {
		err = scheduleDynamicGroupReconcile(ctx, deps, rule, idmusecases.NormalizedNow(now))
	} else {
		_, err = ReconcileDynamicGroup(ctx, deps, rule, idmusecases.NormalizedNow(now))
	}
	return rule, err
}
...
func scheduleDynamicGroupReconcile(ctx context.Context, deps DynamicGroupDeps, rule *groupdomain.DynamicGroupRule, now time.Time) error {
	if deps.JobRepo == nil {
		...
		return idmusecases.AdminEmit(deps.Emit, &idmdomain.DynamicMembershipEvaluated{At: now, TenantID: rule.TenantID, GroupID: rule.GroupID, RuleVersion: rule.Version, AddedCount: result.Added, RemovedCount: result.Removed, UnchangedCount: result.Unchanged, ErrorCount: result.Errors})
	}
...
func DynamicGroupReconcileHandler(deps DynamicGroupDeps) func(context.Context, *jobsdomain.Job) (json.RawMessage, error) {
	return func(ctx context.Context, job *jobsdomain.Job) (json.RawMessage, error) {
		...
		if err := idmusecases.AdminEmit(deps.Emit, &idmdomain.DynamicMembershipEvaluated{At: time.Now().UTC(), TenantID: rule.TenantID, GroupID: rule.GroupID, RuleVersion: rule.Version, AddedCount: result.Added, RemovedCount: result.Removed, UnchangedCount: result.Unchanged, ErrorCount: result.Errors}); err != nil {
			return nil, err
		}
...
func SyncDynamicGroupsForUser(ctx context.Context, deps DynamicGroupDeps, user *userdomain.User, now time.Time) error {
	...
	for _, rule := range rules {
		...
		var existing *groupdomain.GroupMember
		for _, member := range members {
		...
		matched := false
		if rule.Enabled {
			compiled, compileErr := groupdomain.CompileDynamicGroupRule(rule.Expression, defs)
			if compileErr == nil {
				matched, _ = compiled.Evaluate(*user)
			}
		}
		valid := existing != nil && existing.Source == groupdomain.MembershipSourceDynamicRule && existing.RuleVersion != nil && *existing.RuleVersion == rule.Version
		if matched && valid {
			continue
		}
		if existing != nil {
		...
		if matched {
			...
			if _, err := deps.GroupRepo.AddMember(ctx, &groupdomain.GroupMember{GroupID: rule.GroupID, UserID: user.ID, Source: groupdomain.MembershipSourceDynamicRule, RuleVersion: &version, CreatedAt: idmusecases.NormalizedNow(now)}); err != nil {
				return err
			}
		}
	}
...
func ReconcileDynamicGroup(ctx context.Context, deps DynamicGroupDeps, rule *groupdomain.DynamicGroupRule, now time.Time) (DynamicReconcileResult, error) {
	var result DynamicReconcileResult
	members, err := deps.GroupRepo.ListMembersByGroup(ctx, rule.TenantID, rule.GroupID)
	if err != nil {
		return result, err
	}
	current := map[string]*groupdomain.GroupMember{}
	for _, member := range members {
	...
	compiled, err := groupdomain.CompileDynamicGroupRule(rule.Expression, defs)
	if err != nil {
		return result, err
	}
	...
	for _, user := range users {
		...
		if matched {
			version := rule.Version
			if _, err := deps.GroupRepo.AddMember(ctx, &groupdomain.GroupMember{GroupID: rule.GroupID, UserID: user.ID, Source: groupdomain.MembershipSourceDynamicRule, RuleVersion: &version, CreatedAt: now}); err != nil {
				return result, err
			}
			result.Added++
		}
