...
-- name: ListProvisioningTasksByConnectionPageAfter :many
...
-- name: ListProvisioningTasksByConnectionPageBefore :many
SELECT id, tenant_id, connection_id, source_type, source_id, source_version, operation, status, job_id, last_error, created_at, updated_at, completed_at
FROM provisioning_tasks WHERE tenant_id=$1 AND connection_id=$2
  AND (sqlc.arg(filter_status)::text = '' OR status=sqlc.arg(filter_status)::text)
  AND (sqlc.arg(filter_source_type)::text = '' OR source_type=sqlc.arg(filter_source_type)::text)
...
-- name: UpdateProvisioningTaskStatus :exec
...
-- name: RetryDeadLetterProvisioningTask :execrows
...
WITH materialized AS (
  UPDATE provisioning_scheduled_deprovisions AS s SET status='materialized', task_id=sqlc.arg(task_id), updated_at=sqlc.arg(now)
  ...
  SELECT sqlc.arg(task_id), m.tenant_id, m.connection_id, 'user', m.user_id, m.source_version, 'delete', 'pending', sqlc.arg(now), sqlc.arg(now)
  FROM materialized m
