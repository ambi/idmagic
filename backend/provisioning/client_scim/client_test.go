...
//spec:covers RFC7644-OUT-DISCOVERY: `/ServiceProviderConfig` から 5 つの対応可否を読む。
...
//spec:covers RFC7644-OUT-RESOURCE-OPERATIONS: 作成は `POST /Users` で送り、応答の id を相関に使う。
...
//spec:covers RFC7644-OUT-ERROR-RESPONSE: 409 は既存リソースへの関連付けの合図として扱う。
...
//
//spec:covers RFC7644-OUT-PATCH: 下流の `patch.supported` が真のときだけ PATCH で送り、
...
//spec:covers RFC7644-OUT-PATCH: 条件が偽のときは PUT でリソース全体を置換する。
...
//spec:covers RFC7644-OUT-RESOURCE-OPERATIONS: 削除に対する 404 は、目的の状態に既に到達しているものとして成功に数える。
...
//spec:covers RFC7644-OUT-RESOURCE-OPERATIONS: 削除は `DELETE /Users/{id}` で送る。
...
//spec:covers RFC7644-OUT-ERROR-RESPONSE: 429 は `Retry-After` に従う再試行として扱う。
