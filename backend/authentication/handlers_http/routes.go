...
func RegisterRoutes(g *echo.Group, d Deps) {
	...
	g.POST("/api/account/v1/step-up/start", func(c *echo.Context) error { return mfahttp.HandleStartStepUp(d, c) })
	g.POST("/api/account/v1/step-up/complete", func(c *echo.Context) error { return mfahttp.HandleCompleteStepUp(d, c) })
	g.POST("/api/account/v1/step-up/webauthn/challenge", func(c *echo.Context) error { return mfahttp.HandleStepUpWebAuthnChallenge(d, c) })
	g.GET("/api/account/v1/security", func(c *echo.Context) error { return handleGetAccountSecurity(d, c) })
	g.GET("/api/account/v1/notification-preferences", func(c *echo.Context) error {
		return securitynotificationhttp.HandleGetNotificationPreferences(d, c)
	})
	g.PUT("/api/account/v1/notification-preferences", func(c *echo.Context) error {
	...
	g.GET("/api/account/v1/signin-activity", func(c *echo.Context) error { return handleListSignInActivity(d, c) })
	...
	g.POST("/api/account/v1/sessions/revoke-others", func(c *echo.Context) error { return sessionhttp.HandleRevokeOtherAccountSessions(d, c) })
	g.GET("/api/account/v1/trusted-devices", func(c *echo.Context) error { return trusteddevicehttp.HandleListTrustedDevices(d, c) })
	g.POST("/api/account/v1/trusted-devices/:id/revoke", func(c *echo.Context) error { return trusteddevicehttp.HandleRevokeTrustedDevice(d, c) })
	g.POST("/api/account/v1/trusted-devices/revoke-all", func(c *echo.Context) error { return trusteddevicehttp.HandleRevokeAllTrustedDevices(d, c) })
	g.POST("/api/auth/change-password", func(c *echo.Context) error { return passwordhttp.HandleChangePasswordAPI(d, c) })
	g.GET("/api/auth/password-reset-context", func(c *echo.Context) error { return passwordhttp.HandlePasswordResetContext(d, c) })
	g.POST("/api/auth/forgot-password", func(c *echo.Context) error { return passwordhttp.HandleForgotPasswordAPI(d, c) })
	g.POST("/api/auth/reset-password", func(c *echo.Context) error { return passwordhttp.HandleResetPasswordAPI(d, c) })
	g.GET("/api/admin/v1/users/:sub/signin-activity", func(c *echo.Context) error { return handleGetUserSignInActivity(d, c) })
	...
	g.POST("/api/admin/v1/users/:sub/sessions/revoke-all", func(c *echo.Context) error { return sessionhttp.HandleAdminRevokeAllSessions(d, c) })
	g.GET("/api/admin/v1/authentication-event-buckets", func(c *echo.Context) error { return handleListAuthEventBuckets(d, c) })
