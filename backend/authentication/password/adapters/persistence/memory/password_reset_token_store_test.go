...
func TestPasswordResetTokenStoreInvalidatesPreviousTokenForSubject(t *testing.T) {
	...
	for _, hash := range []string{"old", "new"} {
		if err := store.Save(context.Background(), passwordports.PasswordResetTokenRecord{
...
func TestPasswordResetTokenStoreConsumeSucceedsOnceConcurrently(t *testing.T) {
	...
	results := make(chan *passwordports.PasswordResetTokenRecord, 2)
	for range 2 {
		wg.Go(func() {
	...
	expiredRecord := passwordports.PasswordResetTokenRecord{
