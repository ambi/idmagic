...
type AccountMfaDeps struct {
	...
	CredentialRepo webauthnports.WebAuthnCredentialRepository
...
func StartTOTPEnrollment(ctx context.Context, deps AccountMfaDeps, sub string) (*TOTPEnrollmentStart, error) {
	...
	return &TOTPEnrollmentStart{
		Secret:      secret,
		OTPAuthURI:  totpusecases.BuildOTPAuthURI(secret, accountName, issuerLabel),
...
func ConfirmTOTPEnrollment(ctx context.Context, deps AccountMfaDeps, in ConfirmTOTPEnrollmentInput) error {
	now := authusecases.NormalizedNow(in.Now)
	user, err := authusecases.LoadSelfUser(ctx, deps.UserRepo, in.Sub)
	if err != nil {
		return err
	}
	...
	if !totpusecases.VerifyTOTP(secret, strings.TrimSpace(in.Code), now.Unix(), totpusecases.TOTPWindow) {
		return ErrInvalidTOTPCode
	}
	label := totpFactorLabel
	factor := &totpdomain.MfaFactor{
	...
	if deps.Emit != nil {
		deps.Emit(&authdomain.MfaFactorEnrolled{
...
func RemoveTOTPFactor(ctx context.Context, deps AccountMfaDeps, in RemoveTOTPFactorInput) error {
	...
	if !totpusecases.VerifyTOTP(*factor.Secret, strings.TrimSpace(in.Code), now.Unix(), totpusecases.TOTPWindow) {
		return ErrInvalidTOTPCode
	}
	...
	if err := authusecases.SyncMfaEnrolled(ctx, deps.UserRepo, deps.MfaFactorRepo, deps.CredentialRepo, user, now); err != nil {
		return err
	}
	if deps.Emit != nil {
		deps.Emit(&authdomain.MfaFactorRemoved{
			At: now, TenantID: user.TenantID, UserID: user.ID, FactorType: spec.MfaFactorTOTP,
		})
	}
	return nil
