...
type StepUpDeps struct {
	...
	WebAuthn         webauthnusecases.WebAuthnDeps
...
func StepUpStart(
	...
	if user == nil {
		return nil, authusecases.ErrUserNotFound
	}
	if deps.Emit != nil {
...
func CompleteStepUp(ctx context.Context, deps StepUpDeps, in CompleteStepUpInput) error {
	...
	if user == nil {
		return authusecases.ErrUserNotFound
	}
	switch in.Method {
	...
	case StepUpMethodTOTP:
		...
		result, verr := totpusecases.VerifyTOTPFactor(ctx, deps.MfaFactorRepo, in.Sub, in.Code, now)
		if verr != nil {
			return verr
		}
	...
	case StepUpMethodWebAuthn:
		...
		if _, verr := webauthnusecases.FinishWebAuthnAssertion(ctx, deps.WebAuthn, in.SessionID, in.Sub, in.Assertion, now); verr != nil {
			return ErrStepUpFailed
		}
	case StepUpMethodRecoveryCode:
		...
		recoveryDeps := recoveryusecases.RecoveryCodesDeps{UserRepo: deps.UserRepo, RecoveryCodeRepo: deps.RecoveryCodeRepo, Emit: deps.Emit}
		if _, verr := recoveryusecases.ConsumeRecoveryCode(ctx, recoveryDeps, in.Sub, in.Code, now); verr != nil {
			return ErrStepUpFailed
		}
	default:
		return ErrStepUpUnsupportedMethod
