...
func IssueMfaEnrollmentBypass(ctx context.Context, deps MfaEnrollmentDeps, actorID, userID string, ttl time.Duration, now time.Time) (*domain.MfaEnrollmentBypass, error) {
	...
	if expired, err := deps.BypassRepo.ExpireOpen(ctx, user.TenantID, userID, now); err != nil {
		...
		emitMfaEnrollmentEvent(deps.Emit, &authdomain.MfaEnrollmentBypassExpired{At: now, TenantID: user.TenantID, UserID: userID, BypassID: expired.ID})
	}
	if existing, err := deps.BypassRepo.RevokeActive(ctx, user.TenantID, userID, now); err != nil {
		return nil, err
	} else if existing != nil {
		emitMfaEnrollmentEvent(deps.Emit, &authdomain.MfaEnrollmentBypassRevoked{At: now, TenantID: user.TenantID, ActorUserID: actorID, UserID: userID, BypassID: existing.ID})
	}
	...
	emitMfaEnrollmentEvent(deps.Emit, &authdomain.MfaEnrollmentBypassIssued{At: now, TenantID: user.TenantID, ActorUserID: actorID, UserID: userID, BypassID: id, ExpiresAt: bypass.ExpiresAt})
	return bypass, nil
}
...
func RevokeMfaEnrollmentBypass(ctx context.Context, deps MfaEnrollmentDeps, actorID, userID string, now time.Time) error {
	...
	if bypass != nil {
		emitMfaEnrollmentEvent(deps.Emit, &authdomain.MfaEnrollmentBypassRevoked{At: now, TenantID: bypass.TenantID, ActorUserID: actorID, UserID: userID, BypassID: bypass.ID})
	}
	return nil
}
