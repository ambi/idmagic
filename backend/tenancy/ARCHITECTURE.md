...
is User lifecycle management and a system_admin operates from the default control-plane tenant (ADR-032
§6); tenant-scoped roles are deferred to their own model rather than encoded into `roles` as an embedded
tenant ID (ADR-031). State-changing admin requests additionally verify Origin and a CSRF token on top of
...
target's `sub` for audit traceability (ADR-031).
...
cookie path already covers it without widening cookie scope to the root path (ADR-033). Path-prefix
...
so tenant enumeration is not possible from the resolver's response shape alone (ADR-033).
...
ADR-033's original design, which let unprefixed requests fall back to the `default` tenant and offered a
...
(ADR-144). `Subdomain` is only selectable when a deployment configures a base domain; deployments that
...
same breakage as changing `endpoint_style` (ADR-144).
...
opaque key every other table's `tenant_id` FK depends on (ADR-085). The externally exposed vocabulary —
...
Two default-tenant constants follow the same split: `spec.DefaultTenantID` is a fixed UUID (aligned with
ADR-084's seed UUID series) and `spec.DefaultRealm` is the string `"default"` used only where a
tenant must appear in a URL. FK columns that reference `tenants(id)` are UUID-typed, and `tenant_id`
carries no SQL default — every insert must specify tenant_id explicitly, so a missing value fails loudly
instead of silently landing in the default tenant (ADR-085). This is a stricter instance of the repo-wide
[`tenant_id` retention classes](../../ARCHITECTURE.md#2-tenant_id-retention-classes) policy: append-only
...
(ADR-096). Its eight fields (product name, logo, favicon, two brand colors, support link, legal link,
...
`javascript:`, `data:`, and plain `http://` at write time (ADR-096). An earlier version of this design
...
the tenant bears the readability consequence (ADR-097, superseding ADR-096 decision 3).
...
(ADR-096). `GetTenantBranding` always succeeds: missing config, invalid values, or a missing asset all
...
without cross-tenant leakage (ADR-096).
...
because a bug or malicious loop could still exhaust the database before any async warning fires (ADR-134).
...
counters against actual row counts, after which a System Admin can tighten limits deliberately (ADR-134).
