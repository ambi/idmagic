# IdMagic — Go Implementation of IdP
...
- **JWT Signing**: PS256 signature scheme and JWK Set (using RFC 7638 thumbprints for `kid`). Signing keys are scoped per tenant behind a pluggable `KeyProvider`: `/realms/{tenant_id}/jwks` exposes only that tenant's keys, and rotation/disable operate on the requesting tenant. Local (in-memory) and PostgreSQL providers are for development; the `VaultTransit` provider (`KEY_PROVIDER=vault`) keeps private key material inside HashiCorp Vault's Transit engine and delegates signing over its HTTP API, so the app never holds the private key (ADR-075).
- **Persistence**: PostgreSQL for persistent state, Valkey for volatile state.
- **Outbox Pattern**: PostgreSQL outbox tables processed by the Kafka relay (`idmagic-relay`).
...
# 2) In another terminal, start idmagic with SMTP settings
export EMAIL_SENDER=smtp
export SMTP_HOST=127.0.0.1
export SMTP_PORT=1025
export SMTP_TLS=none
export SMTP_FROM=noreply@idmagic.test
./dev.sh
```
...
DATABASE_URL='postgres://idmagic:idmagic@localhost:5432/idmagic?sslmode=disable' \
...
| `KEY_PROVIDER` | `local` / `vault` (unset → local/postgres) | Signing `KeyProvider`. Set `vault` to keep private keys in HashiCorp Vault Transit (ADR-075) |
| `VAULT_ADDR` | URL string | Vault server address. Required if `KEY_PROVIDER=vault` |
| `VAULT_TOKEN` | Token string | Vault token with access to the Transit mount. Required if `KEY_PROVIDER=vault` |
| `VAULT_TRANSIT_MOUNT` | Mount path (`transit`) | Vault Transit secrets-engine mount path |
| `VAULT_KEY_PREFIX` | String (`idmagic-signing-`) | Prefix for the per-tenant Transit key name (`{prefix}{tenant_id}`) |

- **Signing keys are per tenant.** Each tenant has its own key set; `/realms/{tenant_id}/jwks` returns only that tenant's active and still-valid retired keys, so a token signed for tenant A never verifies against tenant B's JWKS. Rotation and disable are scoped to the requesting tenant, and `SigningKeyRotated` carries `tenantId` so the event surfaces in that tenant's audit view.
- **KeyProvider operations (Vault Transit).** Enable the Transit secrets engine (`vault secrets enable transit`) and grant the token `create`/`read`/`update` on `{mount}/keys/*` and `{mount}/sign/*`. Keys are created lazily as RSA-2048 signing keys named `{VAULT_KEY_PREFIX}{tenant_id}`; the app mirrors only the public key for JWKS and delegates PS256 signing to `transit/sign`. If Vault is unreachable the provider fails closed — new issuance/rotation is refused rather than silently falling back. The `local` and `postgres` providers remain available for development and tests; production should set `KEY_PROVIDER=vault`.
...
| Username (admin + system_admin) | `root` |
| Password (both) | `demo-password-1234` |
...
├── cmd/idmagic/main.go                   App entry point
