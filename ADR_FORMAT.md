...
**実際に分岐があり、却下した選択肢が実在するときだけ書く。** 設計を記述したいだけなら ADR を起こさず、
`ARCHITECTURE.md` を更新する。「決定」節に選択肢の比較がなく規則の列挙しかないなら、それは設計文書で
あって決定記録ではない。

## 役割の境界

ADR が持つのは **SCL からも設計文書からも再導出できない「なぜ」**——採用 / 却下した選択肢、判断の前提、
当時の制約、見直し条件、運用上の学習——だけである。次は ADR に書かない。

- **規範要件・振る舞い・契約・データ形状**は SCL（`spec/`）に置く。ADR は結果として変わる SCL 要素を「影響」節で*参照*する（本文を転記しない）。
- **現在の設計**（採用スタック、ディレクトリ規約、依存方向、内部構造、主要フローの動作、データモデル設計）は `ARCHITECTURE.md` に置く。ADR はそこへリンクする。
- **機械検証される module 台帳**は `architecture.yaml` に置く。

「決定」節は**設計の要約に留める**。何を決めたかを数行で述べ、詳細は設計文書を指す。スキーマ・列定義・
フロー詳細・ディレクトリ表を転記しない。逆方向として、設計文書側にはその形になった理由を1〜2文だけ
インラインで添え、選択肢の全文はこの ADR へリンクする（`ARCHITECTURE_FORMAT.md` §3.3）。

**分量の目安**: 本文が 60 行を大きく超えたら、設計内容が混ざっていないか疑う。

既存の ADR が仕様や設計そのものを抱え込んでいると気づいたら、その内容を SCL / `ARCHITECTURE.md` へ移し、ADR 本文には移管先へのポインタと「なぜ」だけを残す。**ADR は削除しない**（上の削除禁止と同じ理由）。移管そのものが非自明な判断を含むなら新しい ADR を起こす（例: ADR-105〜109、ADR-143）。

移管は一括では行わない。**ある context の設計に触れたら、その context 分を移す**（`implement-work-item` の完了ゲート）。
