...
ここにあるスクリプトは、バックアップと災害復旧戦略のうち、`pg_dump` と論理復元の経路を実装する。手順、災害復旧のシナリオ、検証チェックリストは [`docs/runbooks/backup-restore-dr.md`](../../docs/runbooks/backup-restore-dr.md) にあり、この README はスクリプトの実行方法だけを扱う。
