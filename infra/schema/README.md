...
- `postgres.sql` に SQL コメント (`--`) を書かない。理由は独立して 2 つある:
  - 設計上の根拠は DDL ファイルではなく、責務を持つ `spec/` の正本に置く。「なぜ」を言い直すコメントは、意思決定を複製した場合と同じように正本の設計記録からずれる。列型の規則と `tenant_id` の保持区分は `docs/persistence.md` を参照し、ここでは繰り返さない。
...
- 次の規約は設計ではなく SQL の書き方に関するため、このファイルで維持する。これを超える内容は `docs/persistence.md` を参照する:
