...
CI で実行するジョブと検査の一次情報は [`.github/workflows/idmagic-ci.yaml`](../../.github/workflows/idmagic-ci.yaml) である。この文書は一覧を複製せず、失敗を切り分ける原則だけを持つ。ジョブを追加または削除するときはワークフローを変更し、対応する `mise` タスクが無ければ先に `mise.toml` へ追加する。
