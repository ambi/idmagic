...
この文書は、`docs/development/` に収めた開発文書の入口である。開発文書には、開発者が IdMagic を変更、検証、リリースするための手順と、その判断に必要な説明を書く。プロダクトが何をするかは `docs/` 直下と各 Bounded Context の正本文書に、Pull Request の規則はルートの [コントリビューション](../../CONTRIBUTING.md) に書くので、ここへ複製しない。
