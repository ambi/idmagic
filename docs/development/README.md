...
このディレクトリは、開発者が IdMagic を変更、検証、リリースするための手順と、その判断に必要な説明を持つ。プロダクトが何をするかは `docs/` 直下と各 Bounded Context の正本文書が持ち、Pull Request の規則はルートの [コントリビューション](../../CONTRIBUTING.md) が持つため、ここへ複製しない。
