...
参照する記号と規範 ID には `Coverage` の 1 行が付き、宣言があるか、それを名指すファイルがあるかを言う。宣言済みで誰も名指していなければ着手位置は実装であり、宣言がなければ `spec-change` から始まる。名指しは宣言された名前が文字列として現れることであって、その振る舞いに到達できることの保証ではない。
