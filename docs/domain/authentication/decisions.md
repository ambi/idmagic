...
- リセットのトークンは共通のアクショントークン核が発行し検証する ([設計ガイドライン](../../design/application/design-guidelines.md))。この Context が持つのはパスワードという用途別の作用だけであり、目的の束縛、有効期限、ダイジェストだけの保存を自前で書かない。拒否の理由 (用途違い、ダイジェスト不一致、期限切れ、消費済み) は返るエラーの連鎖に残すが、外部のレスポンスでは区別しない。理由を出し分けると、リセットのリンクが有効なアカウントを探る手段になるからである。
