...
- この Context の管理 API のうち、セッションと認証情報に対する操作は API アクセストークンから到達でき、`sessions:read` と `sessions:write` を要求する。MFA 登録の一時免除と認証器のリセットは対象 `User` の資格情報を変えるので `users:write` に属する。認証イベントのバケットは監査の派生的な参照なので `audit:read` に対応させる。
...
- 認証とアイデンティティ管理の設定値 (パスワードの履歴の深さ、漏洩の照合のデフォルト、TOTP / WebAuthn / 復旧コードのパラメーター、リセットのトークンの有効期間、ログインのスロットルの閾値) は、プロダクトの目標に散らさずこのポリシーの節にまとめる。
...
- パスワードを忘れた場合の処理は、単回限りでハッシュ化されたリセットのトークンを発行し、レスポンスを一様にし、メールの配送を最大限努力とする。これによりこの流れが利用者名の存在を暴く手段にも、SMTP の障害を探る手段にもならない。
- リセットのトークンは共通のアクショントークン核が発行し検証する ([設計ガイドライン](../../design/application/design-guidelines.md))。この Context ではパスワードという用途別の作用だけを実装し、目的の束縛、有効期限、ダイジェストだけの保存を自前で書かない。拒否の理由 (用途違い、ダイジェスト不一致、期限切れ、消費済み) は返るエラーの連鎖に残すが、外部のレスポンスでは区別しない。理由を出し分けると、リセットのリンクが有効なアカウントを探る手段になるからである。
- トークンの使用済み化とパスワードおよび履歴の保存は 1 つのトランザクションで確定する。分けると、同じリンクで二度パスワードが変わる状態と、パスワード規則や履歴の照合で拒否された利用者がリンクだけを失う状態のどちらかが起きる。後者は、正当な利用者から回復手段を奪うという点で拒否そのものより重い。したがって作用の前にトークンを消費せず、作用が失敗したときはトークンを未使用のまま残す。
...
- 信頼済みデバイスは端末の指紋ではなく、selector と verifier に分けたサーバー発行のシークレットで識別し、利用のたびに回転させる。指紋は攻撃者に複製でき、正規の利用者側では勝手に変わるからである。
...
- 信頼済みデバイスはテナントごとの明示的な有効期間 (`trusted_device_max_age_seconds`) でのみ有効になり、デフォルトは無効である。
- セキュリティ通知は、イベントの型ではなく監査の射影と同じワイヤ表現の上で動くディスパッチャーが、共通の配信点から送る。カタログはコードではなくデータであり、通知の仕組みは各ドメインパッケージへ依存しない。
- 通知は最大限努力であり、送信は配信点から切り離す。SMTP の待ち時間でログインのレスポンスを延ばさないためであり、送信の失敗が元の操作を巻き戻すことは無い。配送の outbox・再送・dead-letter は持たない。これより重要度の高いパスワードのリセットとメールアドレスの検証が同期の最大限努力である以上、通知にだけ配送基盤を設けても一貫性が無い。
...
- この Context を `Core` に分類する。標準が決めるのはプロトコルだけで、ステップアップ、信頼済みデバイス、登録の強制、復旧はこのプロダクトが定義するポリシーである。委譲すれば IdP であることをやめることになるため、`Generic` の判断が適用できない。
