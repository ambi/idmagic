...
記録の正は管理しない。User と Group の正の記録は `IdManagement` で、Application の割り当ての正の記録は `Application` で管理する。`IdGovernance` は User のライフサイクルイベントを購読し、冪等なコマンドインターフェースを介してこれら記録系 Context の状態を変更する。
