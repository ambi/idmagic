...
**拒否した要求はハンドラーへ到達しない。** 判定は routing の後、ハンドラーの手前で行う。経路の分類にルートパターンが要るので routing より後に置き、状態を一切変えないために ハンドラーより前に置く。ロードシェディング順序のステージ 5 が求める「状態を部分的に更新しない」は、ここでは何も実行しないという形で成り立つ。

分類は経路の登録と同じ場所に定義し、実行時ではなく組み立て時に決まる。分類の無い経路は、組み立てた router の全量に対する網羅性の検査が拒む。それでも実行時に分類の無い経路へ到達した場合は `interactive_auth` と同じ上限で扱う。本番で誤って拒否するほうが、誤って通すより高くつくからである。その到達自体はメトリクスに `unclassified` として現れる。
