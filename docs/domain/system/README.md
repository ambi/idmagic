...
プロダクト全体が従う外部規範は [全体の標準仕様](../standards.md)で、Context を跨ぐ語は [用語集](../glossary.md)で、実行単位は [ランタイムアーキテクチャ](../../design/architecture/runtime.md)で、信頼境界は [脅威モデル](../../design/security/threat-model.md)で定める。実行手順と検証コマンドは仕様ではなく、リポジトリの `README.md` にある。
...
| [System のシナリオ](scenarios.feature.md) | 受け入れシナリオ |
