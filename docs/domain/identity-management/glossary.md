...
| ReservedRole | 製品が意味を定めており、割り当てられる対象を製品側の規則で限定するロール名。`system_admin` だけがこれに当たり、制御面テナントの User と Group にだけ新しく割り当てられる。Agent にはどのテナントでも割り当てられない。これ以外のロール名はテナントが自由に決める文字列として扱う。 | 予約ロール |
