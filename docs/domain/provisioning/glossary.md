...
| RemoteResourceLink | IdMagic の `User` または `Group` と、下流の SCIM サービスプロバイダー上のリソース（リモート ID、`externalId`、`etag`）との対応を保持するエンティティ。HTTP 409（既存リソースとの衝突）では一致判定の属性を使って既存リソースへ関連付け、HTTP 404（リソースの消失）では再作成して関連付けを更新する。下流で有効として反映したかも記録し、照合が User の有効状態と比べる。 | remote link, 相関 |
...
| Reconciliation | 接続ごとに、User のあるべき状態と `RemoteResourceLink` が記録する反映済みの状態を突き合わせ、差分をプロビジョニングタスクにする定期処理。書き込み時の捕捉が作らなかった差分を回収する。 | reconciliation, 照合 |
