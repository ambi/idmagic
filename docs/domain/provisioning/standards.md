...
| RFC7643-OUT-GROUP-RESOURCES | partial | MUST | 接続の `push_groups` が有効なとき、Group を SCIM の Group リソースとして送る。本文は必須属性 `schemas` (`urn:ietf:params:scim:schemas:core:2.0:Group` の 1 要素) と、接続の属性対応付けが解決した属性で組み立てる。`displayName` の取得元は `GroupPushConfig.display_name_source` が選ぶ。選べるのは Group の名前、説明、メールアドレスの 3 つで、デフォルトは名前である。選んだ属性を Group が持たないときは名前へ落とし、配信は失敗させない —— 表示名は fail-closed の判断ではない。メンバーシップはリソース本文には載せず、`members` に対する増分 `add` の PATCH として別に送る。送るのは既に下流へ provision 済みのメンバーだけで、対応関係を持たないメンバーの識別子をこちら側で作ることはしない。メンバーの除去は送らない —— 下流の現在のメンバー集合を読み戻していないので、除くべき相手を知る手段が無く、全置換はこの接続が追加していないメンバーを消す。 |
