...
下流の SaaS へユーザーとグループを反映する、外向きのプロビジョニングを担う。情報の正は IdMagic 側の User と Group であり、下流のリソースはその複製である。接続は Application 1 件につき最大 1 件とし、プロビジョニングの対象範囲には既存の ApplicationAssignment を利用する。
