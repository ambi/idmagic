...
ストアへ到達できない場合は、すべてのポリシーでフェイルクローズに拒否する。拒否は閾値超過と同じ `RateLimitedError` を伴う HTTP 429 とし、固定の短い `Retry-After` を返す。保護対象の操作はどれも 500 を宣言しておらず、読めなかった時間枠から残り時間を計算することもできないためである。保護対象のエンドポイントはすでに PostgreSQL を必須としているため、依存先の種類は増えない。
