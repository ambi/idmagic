...
API レプリカ数の自動調整は、この式が与える数を境界として使う。下限は可用性のために 3、上限は上の参照構成が与える 30 とし、判定は CPU 使用率で行う。CPU は式が使う持続処理能力の代理指標にすぎないので、実測で 1 レプリカ当たり持続処理能力を置き換えたときは上限も置き換える。上限を 30 で止めるのは、レプリカ数がそのまま論理接続予算に乗るためである。上限を上げるときは、後述の接続予算と 70% 規則をともに満たすことを先に確かめる。
...
ステージ 1 とステージ 2 はワーカーの実行レーンが実現する。ステージ 3 からステージ 5 は API プロセスの入場制御が実現し、経路をどのクラスへ割り当てるかと拒否の形は [contexts/system/decisions.md](contexts/system/decisions.md#load-shedding-by-priority-class) と [deployment.md](deployment.md#load-shedding-under-saturation) が持つ。

Discovery と JWKS は正となる状態から導出できるためキャッシュ済み応答を利用できるが、鍵またはテナント設定の失効と更新を TTL だけに依存させない。キャッシュ可能であることは縮退時に拒否してよい理由にならない。ヒット率は保証値ではなく、キャッシュが空の間の API 到達率は上の Peak request profile が示すとおり 10 倍になり、そこで JWKS を返せなければ依存先はトークンを検証できない。障害種別ごとの遷移、冗長化方式、過負荷保護の実装、運用手順は高可用性設計で定める。
