...
平常時に 1 件も拒否が出ない状態へ戻ったことを `rate(http_admission_decisions_total{outcome="shed"}[5m])` で確かめる。閾値を触った場合は、デフォルト値がまだ測っていない仮定値であることを踏まえ、キャパシティ検証で置き直すまでの暫定であることを記録に残す。
