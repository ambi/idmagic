...
4. **直近のデプロイと設定変更。** リリース、テナント設定、鍵のローテーションのいずれかが直前にあるか。
...
- キャパシティが原因なら [ロードシェディング順序](../design/performance/capacity.md#ロードシェディング順序) に従う。`bulk` レーンと保守バッチを先に止め、`/token` の枠を残す。**順序を飛ばして `/token` を落とさない。**
