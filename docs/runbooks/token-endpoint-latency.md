...
2. **PostgreSQL の接続枠。** [構成算出規則](../design/performance/capacity.md#構成算出規則) の接続予算を超えると、待ちが遅延として現れる。プールの待機数と、レプリカ数 × プール上限が物理容量の 70% 以下に収まっているかを確認する。
...
- 滞留なら API レプリカを増やす。必要数の算出は [構成算出規則](../design/performance/capacity.md#構成算出規則) にある。**接続予算を同時に見直さないと、レプリカを足して PostgreSQL 側で詰まる。**
- 特定テナントの急増が原因なら、そのテナントの流量制限を絞る。全体の閾値を下げない。
- 直近のリリースと相関するなら後退する。
- 容量不足なら [縮退順序](../design/performance/capacity.md#縮退順序) に従う。管理用の集計、エクスポート、再同期を先に止める。
