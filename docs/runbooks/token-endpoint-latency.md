...
`TokenLatencyBudgetBurn`。`/token` の p99 が 5 分にわたり 300 ms を超えたときに発火する。燃やしているのは [SLO-TOKEN-LATENCY](../requirements/quality.md#サービス目標) の error budget である。

エラー率のアラートが同時に出ているなら、先に [`/token` のエラー率](token-endpoint-error-rate.md) を見る。**失敗が遅延として現れているだけの場合があり、そのときは遅さを追っても原因に届かない。**
...
2. **PostgreSQL の接続枠。** [サイジング計算式](../design/performance/capacity.md#サイジング計算式) の接続予算を超えると、待ちが遅延として現れる。プールの待機数と、レプリカ数 × プール上限が物理キャパシティの 70% 以下に収まっているかを確認する。
...
- 滞留なら API レプリカを増やす。必要数の算出は [サイジング計算式](../design/performance/capacity.md#サイジング計算式) にある。**接続予算を同時に見直さないと、レプリカを足して PostgreSQL 側で詰まる。**
- 特定テナントの急増が原因なら、そのテナントの流量制限を絞る。全体の閾値を下げない。
- 直近のリリースと相関するなら後退する。
- キャパシティ不足なら [ロードシェディング順序](../design/performance/capacity.md#ロードシェディング順序) に従う。管理用の集計、エクスポート、再同期を先に止める。
