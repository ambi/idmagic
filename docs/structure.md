...
HTTP ルーティングは `backend/shared/http/server_http/routes.go` で組み立てる。ここがテナント単位のルートを既定のテナントと `/realms/:tenant_id` の両方に登録する。制御面のテナント横断操作も同じグループに登録し、制御面テナントへの限定はルーティングの接頭辞ではなくハンドラー側の制御面主体の判定が担う。
