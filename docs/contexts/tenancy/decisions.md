...
- この Context を `Supporting` に分類する。テナント境界はあらゆる Aggregate が属する境界であり、壊れたときの被害は最大だが、マルチテナントであること自体は競合との差にならない。ここを `Core` にすると、区分が差別化の所在ではなく重要度の順位に変わってしまう。テナント境界の拒否を最も強く検証することは、この区分ではなく `docs/threat-model.md` と work item の `risk` が要求する。
