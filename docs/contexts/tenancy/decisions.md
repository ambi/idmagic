...
- セキュリティポリシーの上書きは、プロダクトデフォルトを厳しくする方向にだけ許す。解決に失敗したときもデフォルトへ退避せず拒否する。退避すると、テナントが意図して狭めた認可境界を黙って広げてしまうからである。
...
- この Context を `Supporting` に分類する。テナント境界はあらゆる Aggregate が属する境界であり、壊れたときの被害は最大だが、マルチテナントであること自体は競合との差にならない。ここを `Core` にすると、区分が差別化の所在ではなく重要度の順位に変わってしまう。テナント境界の拒否を最も強く検証することは、この区分ではなく `docs/design/security/threat-model.md` と work item の `risk` が要求する。
