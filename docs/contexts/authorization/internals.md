...
受理するのは正規形だけである。復号できることを条件にすると、同じテナントと版を表す文字列が無数に通ってしまう。base64 の復号は入力中の改行を読み飛ばし、末尾の余剰ビットもデフォルトでは無視するうえ、版の側も先行ゼロを許してしまうからである。判定は符号化し直したものと入力の一致で行い、この 3 種の余地をまとめて閉じる。
