...
- メールアドレス変更の確認トークンは共通のアクショントークン核が発行し検証する ([設計規則](../../design/application/design-rules.md))。この Context が持つのは新しいアドレスという用途別のペイロードと、`User` の primary email を確定する作用だけである。トークンの使用済み化とその確定は 1 つのトランザクションで行う。分けると、同じリンクで二度アドレスが変わる状態と、起票から確定までの間に別の利用者が同じアドレスを取ったために拒否された利用者がリンクだけを失う状態のどちらかが起きる。相手がアドレスを手放せば同じリンクがまだ使えるべきなので、作用が失敗したときはトークンを未使用のまま残す。
