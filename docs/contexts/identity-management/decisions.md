...
- この Context を `Core` に分類する。人間と Agent を同じ台帳の主体として扱い、所有者、目的、ライフサイクルを与えることが製品の主張そのものであり、動的メンバーシップ規則、CSV の往復、Tombstone とカスケードを抱えてモデルも複雑である。委譲できる相手が存在しない。
- メールアドレス変更の確認トークンは共通のアクショントークン核が発行し検証する ([design-rules.md](../../design-rules.md))。この Context が持つのは新しいアドレスという用途別のペイロードと、`User` の primary email を確定する作用だけである。トークンの使用済み化とその確定は 1 つのトランザクションで行う。分けると、同じリンクで二度アドレスが変わる状態と、起票から確定までの間に別の利用者が同じアドレスを取ったために拒否された利用者がリンクだけを失う状態のどちらかが起きる。相手がアドレスを手放せば同じリンクがまだ使えるべきなので、作用が失敗したときはトークンを未使用のまま残す。
