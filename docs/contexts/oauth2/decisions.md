...
- RP-Initiated Logout の `/end_session` は、ブラウザー遷移で使う GET とフォーム送信で使う POST の両方を提供する。同じログアウト処理へ到達するが HTTP binding が異なるため、公開契約ではそれぞれに一意の `operationId` を与える。
- トークン、コード、PAR の TTL、レート制限、DPoP のリプレイ防止期間、同意の保持期間は、可用性 SLO ではなくプロトコルとセキュリティの設定であるため、プロダクト目標にはせず 1 か所にまとめる。
...
- この Context を `Core` に分類する。プロトコル自体は標準だが、ここが持つ委譲、Agent の監督、トークン交換、承認リクエストは標準の外にあるプロダクトの判断である。規範シナリオと採用標準の量も最大であり、モデルの複雑さと差別化が同時に高い。
