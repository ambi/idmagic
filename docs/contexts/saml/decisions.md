...
- SSO、SLO、メタデータ、署名証明書は、テナント既定プロファイルの標準探索経路と、名前付き IdP プロファイルの経路をどちらも提供する。SSO と SLO はさらに HTTP-Redirect 用の GET と HTTP-POST binding 用の POST を分ける。同じプロトコル操作でも HTTP の method と path が異なるため、公開契約では各経路に一意の `operationId` を与える。
