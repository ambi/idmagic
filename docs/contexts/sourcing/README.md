...
外部の権威ある取り込み元から IdMagic へアイデンティティを取り込む責務を担う。情報の正は外部にあり、IdMagic 内部のプリンシパルはその写しである。取り込み元との関連付け、外部の不変 ID との相関、取り込み処理とカーソル、外部の状態に従う削除・無効化の規則を定め、取り込み元ごとに機能単位を設ける。
