...
- この Context を `Supporting` に分類する。環境ごとの投入は運用に固有だが差別化はせず、記録の正も持たない。設計の投資先は投入の表現力ではなく、計画と適用の安全性に限る。
- この Context も Aggregate を持たない。`SeedManifest` と `SeedPlan` は 1 回の実行の中でしか生きない値であり、投入した業務データの記録の正は各 Context に残る。Seeding が状態を持つと、投入結果の正が 2 か所に生まれる。
