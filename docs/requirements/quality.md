...
セキュリティとアクセシビリティの義務は[全体の標準仕様](../standards.md)と各 Context の標準仕様が持つ。

## 証拠の区分

- **Specification target**：満たすべきサービス目標またはキャパシティ受入目標であり、未達は構成または実装の改善を要する。
...
[リファレンスワークロードプロファイル](../design/performance/capacity.md)のデータを投入し、ピークリクエストを同時に 15 分間ウォームアップした後、60 分間処理する。
...
ローカルの復旧ドリルは経路の動作を確かめるが、本番の RPO と RTO を確定する証拠ではない。
