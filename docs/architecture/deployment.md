...
本書は、[ランタイムアーキテクチャ](runtime.md)が宣言する実行単位を、どこへ置き、どうつなぐかを示す。
`infra/` の下にある Docker Compose ファイル、Kubernetes マニフェスト、Terraform を、本書では**構成ファイル**と呼ぶ。
...
キャパシティ算出の設計入力である[リファレンスワークロードプロファイル](../design/performance/capacity.md#リファレンスワークロードプロファイル)とは別の概念である。
