...
[Specification Format](../../../SPECIFICATION_FORMAT.md) は「拒否が書かれていない制御には、照合すべき記述がない」と述べる。同じ形が一段上にも当てはまる。**脅威が書かれていなければ、制御の欠落は何の記述とも矛盾しない。** `mise run check-security-controls` は宣言された拒否がテストされていることを確かめるが、宣言そのものが無い制御については何も言わない。
...
分類は STRIDE を用いる。脅威の見落としを防ぐ網としては十分に粗く、このプロダクトの関心を素直に覆うからである。攻撃木と攻撃ライブラリは、網羅性の主張が弱く維持の費用が高いので採らない。`Category` 列の値は次の 6 つに限る。
...
| `accepted` | 制御を設けないことを意図して選んだ。理由と再検討の条件を [Accepted residual risk](#受容した残留リスク) に書く |
...
**是正を担う work item はこの表に書かない。** 対応する work item の側で `THREAT-NNN` を名指しする。向きを一方向に保つのは、一次情報文書が現在の状態を書く場所であり、まだ存在しない制御への前方参照を書くと、work item が完了して移動した時点で参照が黙って腐るからである。どの work item がある脅威を担っているかは `mise run spec-where THREAT-NNN` で引ける。
...
| 境界 | ここで信用しないもの |
|---|---|
| ブラウザーとゲートウェイ | リクエストの出所。セッション Cookie の存在は、そのリクエストがプロダクト自身の UI から出たことを証明しない |
...
| THREAT-019 | Elevation of privilege | 非同期の実行が処理の途中で別テナントの範囲へ到達する | Jobs | REQ-JOBS-006、docs/design/security/authorization.md: テナント境界 | `covered` |
...
| THREAT-023 | Information disclosure | 管理者が別テナントの監査記録を検索できる | Audit | REQ-AUDIT-001、docs/design/security/authorization.md: テナント境界 | `covered` |
...
| THREAT-031 | Elevation of privilege | 判定に必要な事実が得られない状況が、許可として扱われる | Authorization | REQ-AUTHORIZATION-005、AUTHZEN-FGA-FAIL-CLOSED、docs/design/security/authorization.md: その他の境界の規則 | `covered` |
...
| THREAT-044 | Spoofing | どの発行者からのレスポンスかを区別できず、別の発行者のレスポンスを受け入れる | OAuth2 | RFC9207-ISS | `covered` |
...
`accepted` は、制御を設けないことを意図して選んだ行である。それぞれについて、選んだ理由と、その選択を見直すべき条件を書く。**条件が書かれていない受容は、受容ではなく放置である。**
