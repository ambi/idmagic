...
- **長さの上限がある文字列**：`TEXT` + `CHECK (char_length(col) <= N)` を使う。`varchar(N)` は使わない。上限を宣言と別の場所に置かず、他の `CHECK` と同じ書き方で並べるためである。`N` の決め方は [文字列長の上限](../application/api-rules.md#文字列長の上限) に従う。書式が固定された識別子は `CHECK (... ~ regex)` で併せて守る。
