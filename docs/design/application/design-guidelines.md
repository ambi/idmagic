...
この文書は、プロダクト全体の設計で、サブドメインの区分、Aggregate の境界、モジュールのインターフェース、Seam、型、作用、エラーをどう評価するかを定める。ディレクトリと依存方向は [構造](../../structure.md)、個別の判断と理由は各 `decisions.md`、コードから復元できない仕組みは各 `internals.md` に書くので、この文書には重ねて書かない。
