# idp-ADR-055: Model Context Protocol (MCP) の認可サーバーとして振る舞う
...
idmagic は既に OAuth 2.1 相当 (PKCE 必須 [[ADR-002]]・DPoP [[ADR-005]]・PAR)、Discovery
...
client 登録ではスケールしない。本 ADR は idmagic を MCP の Authorization Server と位置づけ、
...
2. **Protected Resource Metadata (RFC 9728) を配信し、MCP リソースサーバー登録モデルを設ける**。
   ...
   authorization server (= idmagic issuer)・サポート scope を広告する。配信内容は [[ADR-011]] と
...
5. **既存の AS メタデータ (RFC 8414) / Discovery と DCR (RFC 7591) を MCP クライアントの
   自動オンボーディングに再利用する**。RFC 9728 が指す authorization server を idmagic の
...
- 管理 API / UI に MCP リソースサーバー registry (一覧・登録・更新・削除) が `AdminMcpResourcesManage`
  保護で加わる。
- MCP サーバー (ツール側) 実装・MCP transport は対象外で、idmagic は認可サーバー / metadata 提供側に
  徹する。
