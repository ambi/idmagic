# idp-ADR-081: テナントデフォルトサインインポリシーと上書きモデル

## ステータス

採用。Application context の `models.TenantDefaultSignInPolicy`、
...
1. **所有と語彙。** ApplicationCatalog が tenant 単位で `TenantDefaultSignInPolicy` を所有する。
   ...
   語彙・条件モデルは再設計しない (idp-wi-115 Out of Scope)。デフォルトポリシーは Application の
