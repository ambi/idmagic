# idp-ADR-056: Identity Assertion Authorization Grant による企業管理型 Cross-App Access を仲介する
...
idmagic は既に Token Exchange grant ([[ADR-049]] / [[wi-50]])、Resource Indicators (RFC 8707) による
...
- **仲介を外部 IdP (Okta 等) のみに依存する**: 外部 IdP に XAA を委ねれば自前実装は不要だが、idmagic が
  自身の信頼境界内で broker として振る舞えず、identity assertion の検証・許可ポリシー・失効の主導権を
  外部に預けることになる。idmagic はブローカたりうるべきであり、本 grant を自前で実装する。外部 IdP との
