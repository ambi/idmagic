...
### 実測値（wi-177、authentication context 横展開）

authentication の MFA factor、password history、WebAuthn credential、recovery code、email
change token、authentication event bucket の全 24 クエリは、固定キーまたは固定長の
INSERT / UPDATE / DELETE であり、すべて sqlc 静的生成へ移行できた。動的エスケープハッチは
0 件（100%）である。可変条件を持つ認証イベントの集約・検索は OAuth2 側の audit query が
所有し、authentication 自身の repository には存在しない。

この実測でも sqlc 継続採用の判断を維持する。bob への切替条件である「動的クエリが支配的」
は満たさない。

### 実測値（wi-178、identity-management context 横展開）

User / Group（membership 含む）/ Agent（credential binding 含む）/
TenantUserAttributeSchema の全 26 クエリ（users 6、groups 9、agents 8、
tenant_user_attribute_schemas 3）は、tenant_id・id・sub・username・email 等の固定 WHERE
または固定カラムの INSERT/UPDATE/DELETE であり、すべて sqlc 静的生成へ移行できた。
動的エスケープハッチは 0 件（100%）。`lifecycle->>'status' IS DISTINCT FROM 'deleted'`
（tombstone 除外）は可変 WHERE ではなく全クエリ共通の固定述語であり、sqlc の静的 SQL に
そのまま埋め込めた。identity-management は 7 context から被依存を持つ基盤 context だが、
リポジトリ自体の検索条件は tenant/id 系の単純な等価条件が中心であり、動的クエリ比率に
被依存数の影響は見られなかった。
