# idp-ADR-053: エージェントランタイム向け workload identity federation を確定する
...
idmagic は `client_credentials` ([[ADR-008]]) を持つが、これは依然として共有シークレットを
要する。外部 attestation を信頼の起点にした federation 経路は存在しない。一方で外部の
証明を受け入れる以上、検証を誤れば任意のワークロードがエージェント資格情報を得てしまう。
そこで「どの発行者を信頼するか」「外部 subject をどの idmagic プリンシパルへ写すか」
...
3. **token exchange grant ([[ADR-049]], RFC 8693) を交換機構として再利用する**。専用の資格情報
   ...
   idmagic token を発行する。audience 限定・短命既定など [[ADR-049]] の交換規律をそのまま継承する。
...
5. **発行は短命トークンに限定し、long-lived 資格情報を作らない**。federation 経由で発行する
   idmagic token は短命既定とし、refresh token を伴わない。workload の継続稼働は再 attestation
...
7. **idmagic は relying party / federation 側に徹し、SPIRE を同梱・運用しない**。SPIFFE/SPIRE の
...
- **SPIRE server / agent を idmagic に同梱・運用する**: attestation 発行まで抱え込むと scope creep に
  なり、idmagic の責務 (relying party / federation 側) を逸脱する。attestation 発行インフラは
  利用者側に委ね、idmagic は信頼・検証・交換に徹する。
