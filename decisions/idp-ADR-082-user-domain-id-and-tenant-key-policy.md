# idp-ADR-082: User の domain identity と Postgres tenant key 方針

## ステータス

採用。IdentityManagement context の `models.User` を `identity: id` へ変更し
...
idp-wi-120 で導入。ADR-071 (declarative schema with sqldef) を前提とする。
...
持たない (idp-wi-120 Out of Scope)。したがって列リネームは望ましい最終状態の宣言を
