...
- **版の付け方**: パス版 (`/api/admin/v1/...`, `/api/account/v1/...`)。`/v1/` が唯一のパスであり、
  無版パスは提供しない ([[wi-343-consolidate-management-api-to-v1-only]]:
  IdMagic は本 ADR 起票時点で未リリースであり、無版パスに依存する外部クライアントが実在しないため、
  「無版パス = v1 のエイリアス」という当初案は採らず、リリース前に `/v1/` へ一本化した。
  一度リリースした後にパスを変えるのは本 ADR の破壊的変更の定義に該当するため、この一本化は
  リリース前の 1 度限りの移行であり、以後は本 ADR の通常の版管理・非推奨化手順に従う)。
  SCIM は仕様準拠の `/scim/v2/` を維持し、IdMagic の版管理の対象外とする。
...
- **無版パスを `/v1/` の恒久的なエイリアスとして残す**: 却下 (wi-343)。未リリースの間は
  「無版パスに依存する外部クライアントを壊さない」という前提自体が成立せず、エイリアスを
  残すコストだけが将来に残る。リリース後に同じ判断をすればこの一本化はできない
  (それ自体が破壊的変更になる) ため、リリース前の今のうちに行う。
...
- Go: `/api/admin/v1/...` / `/api/account/v1/...` を正規パスとして直接登録する (エイリアス機構は
  なし)、`Deprecation`/`Sunset` ヘッダ付与ミドルウェア。
