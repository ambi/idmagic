...
# idp-ADR-088: アカウント復旧を階層化し、recovery code を最終手段に再位置づける

## コンテキスト

現状 idmagic の TOTP / WebAuthn 喪失時の復旧経路は、ユーザー自身が管理する
backup **recovery code のみ**である（[idp-ADR-087](file:///Users/tn/src/idmagic/decisions/idp-ADR-087-webauthn-phishing-resistant-mfa.md)、
...
この層に自然に乗る。MFA 登録オンボーディング（[idp-wi-127](file:///Users/tn/src/idmagic/work-items/idp-wi-127-mfa-enrollment-onboarding-and-enforcement.md)）
...
操作（[idp-wi-93](file:///Users/tn/src/idmagic/work-items/idp-wi-93-admin-user-impersonation.md) 等）と同じ権限モデル・監査枠組みに揃える。詳細は別 work item で仕様化する。
...
- **メール検証による自動復旧を主経路にする（Keycloak forgot-password 型）**: 実装は容易だが
  ...
  先に埋める方が費用対効果が高い。secondary/recovery email 自体は [idp-wi-41](file:///Users/tn/src/idmagic/work-items/idp-wi-41-secondary-and-recovery-email.md) の範疇に留める。
...
- **既存資産の再利用**: `WebAuthnCredential` の複数登録・`backup_eligible`/`backup_state`、
  `RecoveryCode` 一式、step-up（[idp-ADR-043](file:///Users/tn/src/idmagic/decisions/idp-ADR-043-account-portal-csrf-and-step-up.md)）、
  監査イベント基盤、管理者操作の権限モデルをそのまま使う。
- **関連 work item**: 第 1 層は idp-wi-127 と連携する新 work item、第 2 層は管理者リセットの
