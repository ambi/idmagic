...
## コンテキスト
idmagic の `internal/platform/` は、OS や CPU などのプラットフォーム差分ではなく、
...
（ADR-070 "technical shared context for cross-context adapters" に置き換えられた、2026-06-28）。
SCL の規範振る舞いは変更しない。

1. `idmagic/internal/platform/` を `idmagic/internal/infrastructure/` にリネームする。
...
- `pkg/`: 外部 module に公開する Go library API を表しやすく、idmagic の内部実装を公開したい意図と誤読される。

## 影響
- Go import path は `idmagic/internal/infrastructure/...` へ変わる。
