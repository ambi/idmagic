...
superseded_by: [ADR-092, ADR-133]
---

# ADR-070: technical shared context for cross-context adapters

## コンテキスト
idmagic は RA §3.6 の「層 × 境界づけられたコンテキスト」の格子に合わせて、
...
README と Go import path に反映。SCL の規範振る舞いは変更しない。
...
## 影響
- Go import path は `idmagic/internal/shared/spec` と
  `idmagic/internal/shared/adapters/...` へ変わる。
