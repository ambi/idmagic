...
idmagic-worker                    │ poll
  ...
  └─ Runner ── ClaimBatch <───────┘
       ├─ HandlerRegistry ──> consumer context usecase
       ├─ Heartbeat ────────> lease_expires_at 延長
       └─ Complete / Fail ──> succeeded / queued(retry) / failed
```

現在の `ClaimBatch` は JobKind や tenant を絞らず、due になった全 `queued` Jobと
リース失効済みの `running` Jobを単一の論理キューから取得する。PostgreSQL では
`ORDER BY run_at FOR UPDATE SKIP LOCKED` と同一 statement 内の `running` 更新により、
複数 worker が同じ Job の有効リースを同時に取得しない。1 worker process は空いている
実行枠数だけ batch claim し、既定4枠で handler を並行実行する。複数 process も同じ
queue を水平スケールできる。このため claim 候補は概ね `run_at` の古い順だが、
並行実行、複数 process、同一時刻の Job があるので厳密な開始順・完了順は保証しない。
現在は JobKind 別の priority、実行枠、claim filter を持たず、全 JobKind が同じ
concurrency を共有する。
