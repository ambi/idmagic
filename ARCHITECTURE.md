...
| Runtime & Infrastructure | `backend/cmd/`, `backend/cmd/internal/bootstrap`, `deploy/`, `frontend/`, `docker compose` | 起動、DI、配信、プロセス境界。 |
...
`backend/cmd/idmagic/` の main パッケージは起動処理を担い、起動時 DI は `backend/cmd/internal/bootstrap` が所有する。また、`backend/cmd/idmagic-relay/main.go` は outbox → Kafka リレープロセスを起動するもので、`backend/cmd/idmagic-relay/internal/relay` の `Run()` を呼ぶ。`backend/cmd/idmagic-worker/` の main パッケージは durable job の実行だけを担当する worker プロセスを起動するもので、`backend/jobs` 側の `Run()` を呼ぶ（[ADR-099](decisions/ADR-099-job-worker-execution-model-and-fault-tolerance.md)）。3 プロセスは同一 Go module と bounded context 実装を再利用し、API から独立して水平スケールする。
