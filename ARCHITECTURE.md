...
- technical shared context と context-owned adapter の分離は [ADR-070](decisions/ADR-070-technical-shared-context-for-cross-context-adapters.md) に従う。
...
`backend/bootstrap/deps.go` の `Dependencies` は HTTP 層へ渡す境界の集約で、memory / postgres_valkey / outbox / otel などの runtime 選択を吸収する。新しい port を追加したら、少なくとも次を確認する。
...
永続化 adapter は `backend/shared/adapters/persistence/{memory,postgres,valkey}` にある。port は所有 context 側に置き、実装だけ shared adapter に置く。
