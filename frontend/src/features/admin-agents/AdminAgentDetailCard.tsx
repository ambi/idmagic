...
import { AuthenticationAPIError, deleteAdminAgent, tenantURL } from '../../api'
    ...
    <Card className="overflow-hidden">
        ...
        {showActions ? (
          <div className="mt-4">
            <AdminPaneActions
              ...
              busy={busy || localBusy}
              editHref={
                killed ? undefined : tenantURL(`/admin/agents/${encodeURIComponent(agent.id)}/edit`)
              }
              actions={[
                {
                  label: t.deleteAgent,
      ...
      <section className="border-t border-slate-100 p-5">
        <h3 className="text-xs font-bold uppercase tracking-normal text-slate-400">
          {t.credentialsHeading.replace('{count}', String(agent.client_ids.length))}
        </h3>
        <p className="mt-1 text-xs text-slate-500">{t.credentialsDescription}</p>
        <ul className="mt-3 grid gap-2">
          {agent.client_ids.map((clientID) => (
            <li
              key={clientID}
              className="flex items-center rounded-lg border border-slate-200 bg-white px-3 py-2 text-sm"
            >
              <span className="truncate font-mono text-slate-700">{clientID}</span>
            </li>
          ))}
          {agent.client_ids.length === 0 ? (
