...
describe('NotificationTemplatesTab', () => {
  ...
  it('saves the edited template as a whole subject/text/html set', async () => {
    ...
    expect(url).toBe('/api/admin/v1/tenant/notification-templates/password_reset/ja')
  ...
  it('renders a preview without sending anything', async () => {
    ...
    const [url, init] = fetch.mock.calls[2]
    expect(url).toBe('/api/admin/v1/tenant/notification-templates/password_reset/ja/preview')
  ...
  it('sends a test message without letting the operator choose a recipient', async () => {
    ...
    await waitFor(() => expect(fetch).toHaveBeenCalledTimes(3))
    const [url, init] = fetch.mock.calls[2]
    expect(url).toBe('/api/admin/v1/tenant/notification-templates/password_reset/ja/test')
    expect(init.method).toBe('POST')
    // 宛先を送る手段が無いことを固定する。
