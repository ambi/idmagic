...
export const dataExportDictionary = defineDictionary(
  {
    ...
    userRoundTripNotice:
      '列名を変更せずにエクスポートすると、同じファイルをそのままインポートの事前検証にかけて安全に再適用できます。attr:<key> 列は組み込み拡張属性、custom:<key> 列は現在のテナント属性スキーマから選べます。',
  ...
  },
  {
    ...
    userRoundTripNotice:
      'Keep the column names unchanged to preview and safely reapply the exported file for import. attr:<key> columns are builtin extended attributes; custom:<key> columns come from the current tenant attribute schema.',
