...
import type { AdminJobsDictionary } from './AdminJobsPage.i18n'

// JobsBrowser はジョブの一覧、詳細、取り消しの表示と操作を持つ。
//
// 権限判断はここに無い。どのテナントを扱うかは呼び出し側が渡す listJobs と cancelJob が
// 決め、この部品は結果を描くだけである。テナント側とシステム側で同じ表と同じ状態遷移を
// 使いながら、範囲の決定を経路の側に残すための境界である (wi-462)。
export type JobsBrowserProps = {
  jobs: AdminJob[]
  nextCursor?: string
  // kinds は登録済みの JobKind 一覧。UI で固定せず、読み込んだジョブから導く。
  kinds: string[]
  t: AdminJobsDictionary
  listJobs: (query: AdminJobQuery) => Promise<AdminJobPage>
  cancelJob: (job: AdminJob) => Promise<AdminJob>
  // confirmCancel は取り消し前の確認。対象テナントを再掲するかどうかは呼び出し側が決める。
  confirmCancel: (job: AdminJob) => boolean
  // tenantColumn は一覧に対象テナントの列を常時出すかどうか。横断一覧では必須である。
...
export function statusLabel(status: AdminJobStatus, t: AdminJobsDictionary): string {
  return {
...
export function laneLabel(lane: AdminJobLane, t: AdminJobsDictionary): string {
  return {
...
export function JobsBrowser({
  ...
  async function handleQuery(event: FormEvent<HTMLFormElement>) {
    ...
    setError('')
    try {
      const page = await listJobs(buildQuery())
      setJobs(page.jobs)
  ...
  async function handleLoadMore() {
    ...
    setError('')
    try {
      const page = await listJobs(buildQuery(nextCursor))
  ...
  const columnCount = tenantColumn ? 6 : 5
          ...
          <div className="overflow-x-auto">
            <table className="w-full text-sm">
              <thead className="bg-slate-50 text-left text-xs font-semibold uppercase tracking-wide text-slate-500">
                <tr>
                  {tenantColumn ? <th className="px-4 py-3">{t.tableHeaderTenant}</th> : null}
              ...
              <tbody>
                ...
                {jobs.map((job) => (
                  ...
                  >
                    {tenantColumn ? (
                      <td className="break-all px-4 py-3 font-mono text-xs">{job.tenant_id}</td>
                    ) : null}
...
export function formatDate(value: string, locale: 'ja' | 'en'): string {
  try {
