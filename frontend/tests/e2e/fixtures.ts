...
const verifier = 'idmagic-e2e-pkce-verifier-0123456789abcdefghij'
...
function handleSMTPConnection(socket: Socket): void {
  ...
  write('220 idmagic-e2e-smtp ready')
...
export function metaPage(view: Bun.WebView): Promise<unknown> {
  return view.evaluate(
    'document.querySelector(\'meta[name="idmagic:page"]\')?.getAttribute("content") ?? null',
  )
}
