...
Bun を開発サーバーと本番バンドラーへも広げて Vite を外す案は、試作して測ったうえで見送った。実行環境、パッケージ管理、単体テストがすでに Bun である以上、変換経路まで一つにまとめられれば道具立ては確かに減る。しかし TanStack Router の自動コード分割は Bun 向けの入口を持たず、公開されている生成器とコード分割器はプラグインコンテキストを共有できないため、ルート単位の遅延取得が丸ごと失われる。Bun の CSS バンドラーは `url()` の参照先を必ず data URI として埋め込むので、フォントが内容ハッシュ付きの資産にならず、配信時の `font-src 'self'` とも衝突する。開発サーバーに至っては `@tanstack/router-core` 内部の循環 import を解決できず画面が起動しない。上流でこの三つが解消されたときに再検討する。

再検討するときは次の二つも確かめる。`bun-plugin-tailwind` は Tailwind CSS のコンパイラーを同梱するため、`package.json` が固定する `tailwindcss` とは別の版が CSS を生成しうる。`Bun.build` は `target: "browser"` でも `process.env.NODE_ENV` を置き換えないため、明示しなければ React の開発ビルドがそのまま本番成果物へ入る。どちらもビルドは成功するので、成否だけを見る検査では気づけない。
