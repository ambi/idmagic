...
上流の外部権威から IdMagic へアイデンティティを取り込む責務を所有する。情報の正は外部の取り込み元にあり、IdMagic 内部のプリンシパルはその写しである。取り込み元との関連付け、外部の不変 ID との相関、取り込み処理とカーソル、上流の権威に従う削除・無効化規則をまとめ、取り込み元ごとに機能単位を設ける。現在は SCIM 2.0 サーバーとして `/scim/v2/Users`、`/scim/v2/Groups` などを提供し、外部 IdP（Okta、Google Cloud Identity、Entra ID）からユーザーとグループの同期を受ける `scim` 機能だけを持つ。将来は、閉域の Directory Connector を扱う `directory` と、定期的なファイル取り込みを扱う `feed` を同じ Context に追加する。分類軸は通信の方向や実行時の形ではなく、外部権威と永続的な取り込み元との関連付けの有無である。管理者による CSV インポート（IdManagement）、ログイン時のフェデレーション（Authentication）、下流システムとの台帳照合（Application または Provisioning）はこの Context の対象外とする。

Context のルートはファサードと組み立てだけを持つ薄い形に保つ。共通の取り込み機構は、複数の取り込み元に実在する共通点だけを抽出し、先回りして抽象化しない。
...
- 内向きのアイデンティティ取り込みは、通信方向や実行時の形ではなく、永続的な取り込み元との関連付けを持つ上流の権威があるかどうかで `Sourcing` にまとめる。この区別により、管理者による CSV 取り込みとログイン時のフェデレーションはこの Context の外に留まり、`Inbound` という名前も不要になる。
- SCIM の `DELETE /Users/{id}` は即座に完全削除せず、プラットフォーム既存の論理削除の方針 (`PendingDeletion`、30 日の猶予、その後に匿名化を伴う連鎖的な完全削除) に統合する。設定を誤った、あるいは誤動作した外部の同期が回復不能な PII の喪失を引き起こさないようにするためである。
- SCIM の多値プロフィールデータはプロトコル専用の副次的なストアには保持せず、IdMagic の正式な Aggregate へ投影する。未対応の複合属性と入れ子のグループは、気付かれないデータ損失や認可上の意味を持たないグラフを生まないよう、明示的に拒否する。
