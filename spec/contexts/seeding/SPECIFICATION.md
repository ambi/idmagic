...
環境別の seed プロファイル、投入計画、プレビューと適用の安全方針を所有する運用 Bounded Context である。所有するのは `SeedProfile`、`SeedRequest`、`SeedPlan`、環境ポリシー、ドリフトポリシー、適用順序であり、投入するリソースの意味、検証、永続化は所有しない。それらは記録を所有する各 Context に残り、Seeding はその公開コマンドインターフェースを依存順に呼ぶ。

この分割によって、Context をまたぐ安全性を 1 か所で検証しつつ、不変条件の検査が二重にならないようにする。プロファイルは常に明示的に選択し、本番では `bootstrap` 以外をフェイルクローズで拒否する。
...
構成を環境名から推測することはなく、リクエストまたは CLI で明示しなければならない。本番が受け付けるのは `bootstrap` プロファイルだけで、`demo`、`test`、`performance` は書き込み前にフェイルクローズで拒否する。宛先を誤ったリクエストによって本番にデモ用の資格情報が投入されることを防ぐためである。

プレビューと適用は同じ計画器を共有する。したがってプレビューで見た計画と適用の内容が食い違うことはなく、同じマニフェスト、生成 seed、シークレットのバージョンで再適用しても何も変更しない。手作業によるドリフトはデフォルトでは競合として扱い、上書きしない。明示的な調停は後続の別契約に委ねる。

適用は Context をまたぐ単一トランザクションではなく、依存順に並べた上限付きバッチの冪等なコマンドで行う。`performance` プロファイルのバッチサイズはデフォルト 250、上限 1,000 とする。専用の進捗テーブルは設けず、同じリクエストをプロファイルと生成 seed から導く論理キーと ID で決定的に再現するため、途中で失敗しても再実行すれば残りだけが適用される。直列化には、リクエストキーごとのプロセス内ミューテックスと、プロセス間では既存の接続に対する PostgreSQL アドバイザリーロックを使う。
