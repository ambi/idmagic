...
操作ごとに 1 つ以上の許可スコープを対応付ける。**正本は TypeSpec の operation に付く `x-api-token-scopes` である。** 全 admin operation が宣言を持つことと、宣言したスコープが `ApiTokenScope` 語彙と双方向に対応することは、リポジトリの検査が強制する。散文の側には対応表を持たない。同じ対応を 2 か所に書くと、照合できないまま片方が古くなるためである。
