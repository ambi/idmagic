...
粒度の対応づけは操作ごとに 1 つ以上の許可スコープとして定める。正本は TypeSpec の operation に付く `x-api-token-scopes` であり、それを所有する Context の `Authorization Boundary` が同じ対応をスコープ単位でまとめて述べる。対話セッション限定の操作は `interactive_session` を宣言する。

この対応づけはフェイルクローズである。**どのスコープを要求するかが定まっていない管理 API へは、API アクセストークンからは到達できない。** 対応づけを持たない操作を素通しさせると、新しい管理 API を足した時点で、その 1 本だけがすべてのスコープで通るようになるからである。同じ理由で、対話セッション限定と宣言した操作も、どのスコープでも到達できない。

トークンからは到達できない対話セッション限定の管理 API は次の 4 群である。それぞれ既存のスコープ語彙に対応がなく、いずれもスコープを新設して与えるより人間の管理者に閉じるほうが安全である。

| 対話セッション限定の管理 API | 所有 Context | 理由 |
|---|---|---|
| API アクセストークンの発行・一覧・失効 | ApiTokens | トークンからトークンを発行できると任意のスコープ集合へ昇格できる |
| 外部 IdP 接続の登録・更新・削除・有効化・無効化・接続テスト | Authentication | 攻撃者が管理する IdP を登録できることは認証の迂回そのものである |
| 認可モデルと関係タプルの管理・判定 | Authorization | 対応するスコープ語彙をまだ持たない |
| テナント横断の DEK 健全性一覧 | DataKeys | 対応するスコープ語彙をまだ持たない |
