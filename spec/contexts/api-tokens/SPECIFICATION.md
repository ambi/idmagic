...
粒度の対応づけは操作ごとに 1 つ以上の許可スコープとして定める。**正本は TypeSpec の operation に付く `x-api-token-scopes` である。** どの操作がどのスコープを要求するかを網羅する表はここにも各 Context にも置かない。仕様の散文と契約の注釈に同じ対応を二重に持つと、照合する手段がないまま片方が古びるからである。各 Context の `Authorization Boundary` が述べるのは、その Context がどのスコープ語彙を使うかと、`read` / `write` の割り当てが操作の名前から読み取れない場合の判断とその理由に限る。
...
対話セッション限定にする理由は 2 つのどちらかである。既存のスコープ語彙に対応がなく、スコープを新設して与えるより人間の管理者に閉じるほうが安全な場合 (ReBAC の認可モデルと関係タプル、テナント横断の DEK 健全性)。あるいは、その操作自体が権限の昇格経路になる場合 (トークンの発行はあらゆるスコープ集合を作れる。外部 IdP の登録は任意のユーザーとしてのサインイン、すなわち認証の迂回そのものである)。どちらも、既存のスコープへ畳み込めばそのスコープを持つトークンが黙って昇格能力を得る。
