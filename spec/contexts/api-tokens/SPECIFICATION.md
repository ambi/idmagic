...
ポータル境界のスコープ (`idmagic.admin` / `idmagic.account`) はブラウザーのポータルが提示する通常の OAuth アクセストークンの境界であり、アカウントポータルのトークンで管理 API を呼ぶ経路をフェイルクローズで塞ぐ。API アクセストークンはポータル境界のスコープを持たず、代わりに対象 API のリソースと操作の粒度を持つ。したがって API アクセストークンでは、**呼び出し先の操作が要求する粒度スコープがポータル境界のスコープの代わりになる**。管理 API は `users:read` や `saml:write` など、SCIM API は `scim:users:read` など、発行者本人のアカウント API は `account:read` などを要求する。

操作ごとに 1 つ以上の許可スコープを対応付ける。**正本は TypeSpec の operation に付く `x-api-token-scopes` である。** どの操作がどのスコープを要求するかを網羅する表は、ここにも各 Context にも置かない。散文と契約の注釈に同じ対応を重複して記載すると、両者を照合できないまま片方が古くなるためである。各 Context の `Authorization Boundary` には、その Context が使うスコープ語彙と、操作名から `read` / `write` の割り当てを判断できない場合の結論と理由だけを記載する。

対話セッション限定の操作は `interactive_session` を宣言する。この対応づけはフェイルクローズである。**どのスコープを要求するかが定まっていない管理 API へは、API アクセストークンからは到達できない。** 対応づけを持たない操作を素通しさせると、新しい管理 API を足した時点で、その 1 本だけがすべてのスコープで通るようになるからである。同じ理由で、対話セッション限定と宣言した操作も、どのスコープでも到達できない。
