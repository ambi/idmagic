...
func (r *PasswordHistoryRepository) Add(ctx context.Context, sub, encoded string, now time.Time) error {
	_, err := r.Pool.Exec(ctx, `INSERT INTO password_history (user_id,encoded,created_at) VALUES ($1,$2,$3)`,
		sub, encoded, now)
	return err
}

func (r *PasswordHistoryRepository) DeleteAllForSub(ctx context.Context, sub string) error {
	_, err := r.Pool.Exec(ctx, "DELETE FROM password_history WHERE user_id=$1", sub)
	return err
}
