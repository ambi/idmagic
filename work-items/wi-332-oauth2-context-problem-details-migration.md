...
`spec/contexts/oauth2.yaml` の `InvalidAuthorizationDetailTypeError` は 422) は
...
別箇所だが、SCL 上は同一 model を指す。ここも 403→422 に揃える。
...
- 各 `WriteBrowserError` 呼び出しについて、それが JSON API (ログイン/MFA/同意
  ...
  handler が bind される interface (`spec/contexts/oauth2.yaml` の
