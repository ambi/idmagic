...
- buildは既存`just build-go`/`just build-ui`と`deploy/docker/Dockerfile`を正本にし、version/commit/dateを固定したartifactとimage digestを一度だけ生成する。SBOM/署名のために別buildしてdigestをずらさない。
