...
- **decision**:
  - `docs/domain/identity-management/decisions.md` へ記録する決定 (ゲストユーザーと招待): ゲストを別 aggregate にせず `User` の種別
    ...
    (無効化か削除か。`docs/domain/identity-management/states.md` の `UserLifecycle` と整合)、
...
- 未決定: ローカル資格情報を持つゲスト (外部 IdP を持たない小規模取引先向け) を許すか。
  第 1 段では**許さない** (外部 IdP 委任のみ) とし、需要があれば `docs/domain/identity-management/decisions.md` を改訂して開く。
...
- [ ] T002 [Spec] ゲストユーザーと招待の決定を `docs/domain/identity-management/decisions.md` に記録する (種別方式の理由・認証経路・既定権限・
