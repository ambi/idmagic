...
- PostgreSQLはproduction規模/RPOに必要なPITR（base backup+WAL archive）を正本とし、`pg_dump`はportable export/小規模drill用途に限定する。`deploy/schema/postgres.sql`は復元後の差分検証に使い、空DBへ先にapplyしてdata restoreと競合させない。
