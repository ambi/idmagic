...
- JWT 署名は PS256 / ES256 (`docs/domain/oauth2/standards.md` の `RFC7518-SIGNATURE-ALGORITHMS`) で、これは承認範囲内。
...
- **go**:
  ...
  - HIBP を `docs/domain/standards.md` の決定に従って処理する (無効化する場合は
...
- **非 FIPS モードの既定を変えないのが原則**。Argon2id は Argon2id のままにする。
  ...
  `docs/domain/standards.md` にこの立場を明記する。
...
- **承認外経路の洗い出しを網羅する作業として明示的にタスク化する**。署名・ハッシュ・
  ...
  一覧を `docs/domain/standards.md` に残して検証可能にする。
...
- 手動: (1) 非 FIPS モードで既存の Argon2id ユーザーがログインでき、挙動が変わらないこと、
  ...
  (4) FIPS モードで HIBP を有効化した設定が起動時に拒否されること (`docs/domain/standards.md` の決定に従う)、
...
これは規制準拠のための意図的なトレードオフであり、`docs/domain/standards.md` に明記する。
非 FIPS モードの既定を変えないことで、一般利用者の強度は維持する。
「FIPS モードで動く」を「FIPS 認証済み」と誤読されると虚偽表示になる。README と
`docs/domain/standards.md` で明確に区別し、マーケティング的な表現を避ける。
承認外経路の洗い出しに漏れがあると、モードの意味が失われる。7 面の棚卸しを
`docs/domain/standards.md` に残し、将来の暗号追加時に見直す対象として明示する。
