...
- `mise run check-spec` が、`docs/domain/identity-governance/scenarios.feature.md` の 21 件を `tools/check/example-coverage-debt.json` から外した状態で通る。台帳から外す前に同じ検査が当の id を名指しで落とすことを、消化ごとに観測する。
