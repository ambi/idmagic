...
- **go**:
  ...
  - cmd:
    - 新規 `cmd/idmagic-key-rotator/main.go` を追加。引数:
        --cadence-days   (default 90)
        --grace-days     (default 7)
        --tick-interval  (default 1h: 観測周期)
      起動時に `RotateSigningKey` の最終実行時刻を確認し、`cadence-days` を超えていれば rotate。毎 tick で `PruneExpiredSigningKeys` を 呼んで grace 経過鍵を dispose する。
  - persistence:
    - `deploy/schema/postgres.sql` を更新し、`signing_keys` テーブルに `retired_at TIMESTAMPTZ NULL` と `expires_at TIMESTAMPTZ NULL` を追加。`psqldef --dry-run` で差分をレビューしてからデプロイ前に適用する。 既存行は両カラム NULL で active 扱い。memory adapter にも対応フィールドを反映する。
  - audit:
    - `SigningKeyRotated` (既存) に加え、新規 `SigningKeyArchived` (kid / retiredAt / expiresAt / disposedAt) を `spec/events.json` に追加する。outbox 経由で publish。
- **infra**:
  - `infra/k8s/idmagic-key-rotator.cronjob.yaml` を skeleton として 追加する (1 日 1 回 tick)。本 WI は雛形までで、本番クラスタへの 投入は Phase 8 の操作タスク。
- **documentation**:
...
- scheduler は全 tenant を列挙して最終 rotation 時刻を評価する `backend/cmd/idmagic-key-rotator` とする。複数 replica/CronJob の重複起動に対して PostgreSQL advisory lock または tenant 単位 compare-and-rotate で冪等にし、API process 内 ticker は採らない。
...
- cadence/grace は typed startup config に寄せる。[[wi-103-startup-config-validation-and-reference]] 未完了でも rotator 自身で `grace < cadence`、正値、duration parse を fail-fast 検証できるようにする。

## Tasks
- [ ] T001 [SCL] signing-keys context の lifecycle/objectives に cadence、minimum JWKS overlap、archive transition と `SigningKeyArchived` を記載し、`just scl-render` する。
- [ ] T002 [Domain] `SigningKey` に lifecycle 時刻を追加し、active signing key は各 tenant に1本、verifying 鍵は grace 終了まで JWKS に残る不変条件をテストする。
- [ ] T003 [Port/Usecase] KeyStore に期限到来鍵の列挙/archive 操作を追加し、`RotateSigningKey` の compare-and-rotate と `ArchiveExpiredSigningKeys(now)` を clock 注入で実装する。
- [ ] T004 [Memory] memory KeyStore で tenant 分離、旧鍵 overlap、期限後非公開、archive の往復を実装・テストする。
- [ ] T005 [Postgres] 宣言 schema、queries、sqlc generated code、repository を更新し、既存鍵を壊さない dry-run 差分と concurrent rotation test を追加する。
- [ ] T006 [Process] tenant enumeration・排他・rotate/archive loop・終了 signal を持つ `idmagic-key-rotator` entry point と設定 validation を追加する。
- [ ] T007 [Deploy] 1日周期の CronJob skeleton、ServiceAccount、Secret/config 注入を追加し、同時起動しても1回だけ rotation されることを確認する。
- [ ] T008 [Verify] rotation 前 token を保持したまま新旧 `kid` の JWKS overlap、grace 後の非公開/archive、再起動後の lifecycle を memory/PostgreSQL の両方で検証する。

## Verification
- `just test-go`
  - reason: KeyStore.Rotate が retired/expires を埋めること、PruneExpired が grace 経過鍵だけを消すこと、ListPublicKeys が active+retired を返すこと、cmd の tick で rotate / prune が想定通り発火する こと。
- `just lint-go`
- `just build-go`
- 手動 1: rotator を `--cadence-days=0 --grace-days=0 --tick-interval=1s` で起動し、tick ごとに新 kid が `/jwks` に並び、次の tick で旧 kid が prune されることを確認する。
- 手動 2: rotator を `--cadence-days=0 --grace-days=300s --tick-interval=1s` で起動し、grace 期間中は旧 kid が `/jwks` に残ること、旧 kid で 署名された access token の `/introspect` / RS 側検証が成立すること を確認する。
- 手動 3: PostgreSQL adapter で migrate → rotator 起動 → 再起動して state が永続化されていることを確認する。
