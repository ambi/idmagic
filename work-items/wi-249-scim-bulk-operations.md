...
- 非同期・バックグラウンド実行(RFC は同期応答を前提とする。将来的に
  パフォーマンス上必要なら別途 `docs/domain/sourcing/decisions.md` で検討)。
...
- 部分失敗時の atomicity は [[wi-239-scim-inbound-resource-contract-conformance]] の
  `docs/domain/sourcing/decisions.md` の変更原子性 (validate-first + 補償クリーンアップ)と同じ方針を踏襲し、Bulk 全体を
...
- [ ] T001 [Spec] `/Bulk` の契約(request/response 形状、`bulkId` 解決、
      `failOnErrors`、`ServiceProviderConfig.bulk` 広告値)を `docs/domain/sourcing/standards.md` と `models.tsp` に追加する。
