...
- [x] T008 [Test] cursor 改ざん、tenant 境界、filter 変更、同一 sort key、削除を挟む遷移の test を追加する。
  - handler レベルで Link header 有無・cursor 拒否・別テナント cursor 拒否・次ページ重複なしの test を全 10 interface に追加 (`ListAdminUsers`/`ListGroups` は既存、今回 `ListAgents`/`ListAdminApplications`/`ListApplicationAssignments`/`ListAdminConsents`/`ListAdminOAuth2Clients`/`ListAdminAuditEvents`/`ListAuthenticationEventBuckets`/`ListProvisioningDeliveries` の8 interface分を追加)。各 test file は新規 `_test.go` として追加し、既存 test helper (`newAdminXxxHandler` 等) は編集していない — 既存 helper はどれも `Deps.PaginationCodec` を設定しておらず (`SetNextLink`/`DecodeForQuery` が nil pointer dereference するため cursor pagination の検証に使えない)、各 file 内に `PaginationCodec` を設定した専用 helper を新設した。
