...
# identity lifecycle event に応じた構造化 workflow を安全かつ再試行可能に実行する
...
本 WI は、User の作成・属性変更・status 遷移を trigger とし、テナント管理者が事前定義した
構造化 action を durable job として実行する。定義の revision 固定、trigger の耐久化、action 単位の
checkpoint、dry-run、監査可能な実行履歴を備え、少なくとも一回実行されても観測される最終状態が
一回分と同じになることを保証する。
...
- **Backend — `backend/identitymanagement/`**:
  - workflow aggregate、validator、trigger evaluator、run planner、step executor と repository port を追加する。
  - User mutation の before/after 差分から該当 workflow revision を決定し、User 更新と同じ transaction 境界で WorkflowRun を作成する。
  - memory / PostgreSQL に workflow definition、run、step checkpoint、notification delivery の永続化を実装する。
  - 未 enqueue の queued run を回収して Jobs に渡す dispatcher と `lifecycle_workflow_run` handler を実装する。
- **Backend — action ports**:
  - Group membership、Application assignment、required action、User status、email sender を context 間 port として注入し、HTTP handler の再利用や直接 repository 更新に依存しない。
  - action は同一テナントの resource だけを対象とし、workflow system actor と origin run/step を domain event に残す。
- **HTTP — `/api/admin/lifecycle_workflows`**:
  - list/create/get/update、enable、disable、archive、dry-run、run list/detail API を追加する。
  - definition 更新には revision または ETag 相当の楽観ロックを要求し、同時編集の lost update を拒否する。
- **UI — `frontend/src/features/admin-lifecycle-workflows/`**:
  - workflow 一覧、構造化 editor、validation summary、dry-run plan、run history、step failure detail を追加する。
  - destructive action、無効な参照、disable/archive の影響を保存前に表示する。
...
## Tasks
- [ ] T001 [SCL] IdentityManagement の glossary/models/states/interfaces/authorization/scenarios/objectives/flows を追加する。
- [ ] T002 [SCL] Application の internal desired-state assignment、Jobs の JobKind/handoff、Audit の検索属性を追加する。
- [ ] T003 [Decision] transactional trigger capture、revision、retry/checkpoint、loop suppression、retention、非補償方針を ADR に記録する。
- [ ] T004 [Domain] workflow definition validator、trigger evaluator、run planner、definition/run state machine を実装する。
- [ ] T005 [Persistence] memory / PostgreSQL の definition/revision/run/step/delivery repository、migration、query を実装する。
- [ ] T006 [Trigger] User create/update/status mutation と atomic run creation を統合し、source occurrence dedup を実装する。
- [ ] T007 [Jobs] lifecycle_workflow_run JobKind、即時/periodic dispatcher、handler、per-user serialization、retry/checkpoint を実装する。
- [ ] T008 [Actions] group、application、required action、user status、fixed-template email の idempotent action port/executor を実装する。
- [ ] T009 [Audit/Ops] workflow/run/step event、検索属性、retention cleanup、metrics、PII-safe logging を実装する。
- [ ] T010 [HTTP] CRUD、enable/disable/archive、dry-run、run list/detail/retry API と authorization を実装する。
- [ ] T011 [UI] typed editor、validation、dry-run、run history、step failure detail と ja/en 文言を実装する。
- [ ] T012 [Docs] README、必要なら ARCHITECTURE.md、運用・障害対応手順を同期する。
- [ ] T013 [Verify] SCL、domain、adapter、HTTP、UI、worker E2E、race、tenant isolation を検証する。
