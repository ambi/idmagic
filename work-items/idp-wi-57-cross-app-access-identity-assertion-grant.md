---
id: idp-wi-57-cross-app-access-identity-assertion-grant
title: "Cross-App Access (Identity Assertion Authorization Grant) でエージェントのアプリ間アクセスを仲介する"
...
# Scope
- **decision**: 新規 ADR [[ADR-056]]: 対応する draft 改訂 (draft-ietf-oauth-identity-assertion-authz-grant)、 identity assertion の受理条件 (信頼する issuer / audience)、app A → app B の許可関係 (どの client がどの resource を要求できるか) の登録モデル、MCP Enterprise-Managed Authorization との対応、企業管理者による付与・取消の責務を確定する。
- **scl**: 新規 model: IdentityAssertionGrantRequest / AppAccessGrant / CrossAppAccessPolicy。 token-exchange の subject_token_type に identity assertion 系を追加する。, [object Object], [object Object]
- **go**: identity assertion の検証 → 許可ポリシー照合 → app B 向け audience 限定トークンへの 交換を fail-closed で実装する。Token Exchange / Resource Indicators 基盤を再利用する。
- **http**: token-exchange での identity assertion 受理、app-to-app 許可ポリシー管理 API。
- **ui**: [object Object]
