...
- **API プロセスを認証プレーンと管理プレーンに分けてデプロイすること。** 本 work item では採らない。判断の根拠は Design に残す。将来必要になった場合の条件も Design に書く。
...
**将来採るとしたらの条件**を残しておく。(a) 入場制御と HPA を入れた上で、なお管理系のバーストがログインのサービス目標を侵すことが実測で示される、(b) 管理系トラフィックが認証系と同等以上の規模になる、(c) 6 Pod 以上の常時稼働が無駄にならない規模で運用されている。この 3 つが揃ったとき、本 work item の優先度クラスはそのままプレーンの境界として使える。**分類をコードに置くのは、その移行を安くしておく意味もある。**
