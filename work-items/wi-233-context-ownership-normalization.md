...
## Tasks
- [ ] T001 [Contracts] 現行公開型・event wire・repository の characterization test を追加する。
- [ ] T002 [SigningKeys] context package と module を新設し consumer を移行する。
- [ ] T003 [ClaimMapping] context package と projection contract を新設し consumer を移行する。
- [ ] T004 [Events] context 固有 event を owning package へ移し shared registry を縮小する。
- [ ] T005 [Adapters] memory/PostgreSQL/Vault/crypto と bootstrap DI を再配線する。
- [ ] T006 [Architecture] context map と Architecture realization/dependency を同期する。
- [ ] T007 [Verify] wire compatibility、adapter parity、全検証を通す。
