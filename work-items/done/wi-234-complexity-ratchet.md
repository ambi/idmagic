...
- [x] T005 [Tests] 抽出した presentation/helper/usecase の test を追加する。
  - 対象は T001 訂正時のインベントリで名指しした `AdminAttributeField`・`AssignmentManager`・`CategoryManager`・Go throttle helper 群を核に、分割後に単体 test の無かった純粋ロジック helper を追加した。
  - Go: `login_throttle_test.go` に `acquireLoginThrottle`(allowed/throttled/store_unavailable の3分岐 + metrics 記録)と `correlationHash`(tenant salt 使用時/salt store 無し/salt store エラー時のフォールバック)の直接 test を追加(5 test)。`recordLoginFailure`/`recordFailedLoginBucket`/`extractClientIP`/`writeLoginThrottled` は既存 test (`auth_event_bucket_aggregation_test.go` 等)で既に被覆確認済み。
  - UI: `AdminApplicationsShared.test.ts`(messageOf/parseList/initials/summarizeSignInRule/appRuleFromInputs/signInRuleWeakerThanDefault、16 test)、`AdminApplicationAssignments.test.tsx`(AssignmentList/AssignmentManager の一覧・追加・解除・エラー経路、7 test)、`AdminApplicationCategories.test.tsx`(CategoryManager のトグル・追加・削除・エラー経路、5 test)、`AdminUsersShared.test.ts`(attributeValueToText/groupedAttributeDefs、5 test)、`AdminAttributeField.test.tsx`(boolean/number/date/string_array/readOnly の分岐、6 test、対象を export に変更)を新規追加。
  - Radix DropdownMenu ベースの `Select` は native `<select>` ではなく `fireEvent.change` が効かないため、`SystemShell.test.tsx` の既存パターン(`fireEvent.keyDown(trigger, {key:'Enter'})` → `getByRole('menuitem')` を click)に倣った。
  - 対象外(残 debt として明示): `AdminUserDetailPage`/`AdminGroupDetailPage`/`AdminAgentDetailPage`(専用 test 無し、list page 経由の間接検証も無し)、`ScimTab`(旧 `AdminSettingsPage.tsx` 時代から未検証)、`AdminGroupsShared`/`AdminAgentsShared` の trivial 一行関数(parseRoles/optionalValue)、`AdminGroupDetailCard`/`AdminAgentDetailCard`/`AgentEditorDialog`/`AdminUserDialogs`(list page test 経由で主要経路は間接検証済み)。将来の T005 追い作業として残す。
  - UI unit test: 357 → 396 (+39)。`just verify-ui`/`just verify-go`/`just verify` すべて green。
...
- [x] T007 [Verify] wire/route 挙動不変、coverage 非低下、全検証を確認する。
  - `just verify`(SCL/work-item チェック、Go lint+race test、UI format/lint/typecheck/build/unit test)green。
  - UI test: 357 → 396 (+39)。Go: `backend/oauth2/adapters/http` に +5 test。挙動を狭める変更は無く、いずれもファイル間移動または新規 test 追加のみ。
  - dev サーバー(`./dev.sh memory`)を起動し、Applications/Users/Groups/Agents/Settings の各 admin route が 200 を返すことと、`/authorize` が分割後も想定通りの構造化 OAuth エラーを返すことを curl で確認した。ヘッドレスブラウザ(chromium-cli 等)がこの実行環境に存在しないため、実際の画面描画・クリック操作の目視確認は未実施(React Testing Library による実 DOM マウント + 操作 test が代替エビデンス)。
...
- **Summary**: Applications(2,677行/state81)、Users(2,457行/state44)、Groups(984行/state30)、Agents(918行/state26)、Settings(749行/state22)の5管理画面モノリスを route/resource operation 単位(list/detail/edit/create/import + 共有 presentation/dialog)へ分割し、Go 側は唯一 budget 超過だった `authorize_handler.go`(1,028行)を既存の concern 別ファイル規約に沿って8ファイルへ分割した。wi-232 が導入済みの complexity budget/debt(`ui-page-lines` 400行、`ui-page-local-state` 10個、`go-source-lines` 800行)を全対象ファイルで新パス・実測値に追従させ、budget 内に収まったファイルの debt entry は削除、収まらなかった edit page 系のみ ceiling を実測値へ更新して残した。抽出後に単体 test の無かった presentation/helper(`AdminAttributeField`、`AssignmentManager`、`CategoryManager`、Go throttle helper 群ほか)へ test を追加した。挙動変更は無く、全て behavior-preserving なファイル間移動と re-export の再配線。
- **Verification Results**:
  - `just verify-ui`(format/lint/typecheck/build/unit test) - passed
  - `just verify-go`(golangci-lint + race-enabled test) - passed
  - `just yaml-check-architecture` - passed
  - `just verify` - passed (SCL/work-item チェック、Go lint+race test、UI format/lint/typecheck/build/unit test)
  - UI unit test: 357 → 396 (+39)、Go: `backend/oauth2/adapters/http` に +5 test
  - dev サーバー(`./dev.sh memory`)起動の上、Applications/Users/Groups/Agents/Settings 各 admin route の 200 応答と `/authorize` の構造化 OAuth エラー応答を curl で確認(ヘッドレスブラウザ非対応環境のため画面描画の目視確認は未実施)
- **Affected Guarantees State**: Applications/Users/Groups/Agents/Settings の admin UI と oauth2 `/authorize` 系ハンドラは、責務境界に沿った複数ファイルへ分割された状態が現在の実装。wi-232 の complexity budget/debt の枠組みにより、新規の 400行/10状態(UI)・800行(Go)超過は今後 owner・work item・期限付き debt entry を明示登録しない限り `yaml-check-architecture` が拒否する。
- **Evidence**: Claude Code が macOS 上で、base revision `bf85a4fb`(wi-232 完了後)に本 WI の T001–T007 実装差分を加えた source tree(対象版はこの記録を含む commit tree、コミット `f1b5f4a2`・`2c44048a`・`6a321eb3`)へ `just verify-ui`、`just verify-go`、`just yaml-check-architecture`、`just verify` を各層完了時点で反復実行した。全 recipe が成功し、UI 396 tests・Go lint/race tests・production build が green。dev サーバーでの route 疎通確認ログは一時ファイルのため保存していない(再現手順は本ファイルの T007 記録を参照)。
- **Remaining Debt**: `wi234-ui-page-lines-admin-application-edit-page`・`wi234-ui-page-local-state-admin-application-edit-page`・Users page debt 4件・`wi234-ui-page-local-state-admin-group-edit-page`(計7件、`expires_at: 2026-10-01`)は `ARCHITECTURE.md` に残存。`AdminUserDetailPage`/`AdminGroupDetailPage`/`AdminAgentDetailPage`/`ScimTab` の専用 test 追加は T005 スコープ外として後続へ持ち越し。
