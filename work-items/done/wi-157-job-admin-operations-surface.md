...
affected_spec:
  ...
  - { path: docs/domain/jobs/scenarios.feature.md, requirement: REQ-JOBS-014 }
...
- [x] T001 [Spec] 管理 API、authorization/access、read model、UX を `Jobs` context に追加する。
  - REQ-JOBS-012 / REQ-JOBS-013 / REQ-JOBS-014 を `docs/domain/jobs/scenarios.feature.md` に追加。`ListJobs` / `GetJob` / `CancelJob` を `main.tsp` に、`AdminJobResponse` / `AdminJobQuery` / `AdminJobListResponse` / `JobNotFoundError` / `JobNotCancelableError` を `models.tsp` に追加。
