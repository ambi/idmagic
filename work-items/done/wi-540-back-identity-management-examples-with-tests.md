...
initial_context:
  specification:
    - docs/domain/identity-management/scenarios.feature.md
  typespec: []
  source:
...
[[wi-496-burn-down-the-example-coverage-debt]] は具体例の被覆台帳の消化単位を Context と決め、測定のうえで残りを Context ごとの子 work item へ割った。本項目はそのうち `docs/domain/identity-management/scenarios.feature.md` が宣言する 66 件を引き取る。
...
- `mise run check-spec` が、`docs/domain/identity-management/scenarios.feature.md` の 66 件を `tools/check/example-coverage-debt.json` から外した状態で通る。台帳から外す前に同じ検査が当の id を名指しで落とすことを、消化ごとに観測する。
...
- **Acceptance RED Evidence**:
  ...
  - **Observed Failure**: 対象 66 件を台帳から外した状態で、66 件すべてを名指しで落とした。例:
    `docs/domain/identity-management/scenarios.feature.md:7: EX-IDMANAGEMENT-001-01 is declared, but no test names it.`
