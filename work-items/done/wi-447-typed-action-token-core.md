...
1. 既存二用途の仕様へ共通不変条件と先読み安全性を追加する。TypeSpec の操作、要求本文、状態コード、エラー契約はいずれも変わらないので更新しない。
...
- **互換性**。HTTP の経路、要求本文、状態コード、エラーコードは変わらない。UI の変更も要らない。
...
- **Summary**:
  ...
  TypeSpec の操作、要求本文、状態コード、エラー契約はいずれも変わっていない。
