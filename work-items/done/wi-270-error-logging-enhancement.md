...
- **Summary**:
  共通 logging middleware と error handler を拡張し、400 validation error を Warn、
  500 error を Error、正常 request を Info として記録するようにした。
- **Verification Results**:
  - `just dev-api` — `GET /health`、404、不正 JSON のログレベルと内容を確認
  - `just test-go` — passed
