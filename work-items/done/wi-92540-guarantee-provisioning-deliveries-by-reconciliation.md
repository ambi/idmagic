...
# Provisioning のプロビジョニングタスクを、書き込み時の捕捉ではなく定期的な照合で保証する

## 動機

`REQ-PLATFORM-003` は「プロビジョニングタスクの行は発火元の変更と同時にコミットまたはロールバックする」と定めている。
...
| 観察 | 内容 |
| --- | --- |
| 捕捉が経路ごと抜けている | LifecycleWorkflow による有効化と無効化（`idgovernance/usecases/lifecycle_workflow_dispatcher.go`）、SCIM による取り込み（`sourcing/scim/usecases/users.go`）、CSV インポート、利用者自身のプロフィール変更は、User を書き換えてもプロビジョニングタスクの行を作らない。障害がなくても、毎回確実に下流とずれる |
...
下流へ送る属性はプロビジョニングタスクの時点の User から読み、`RemoteResourceLink` が下流に何があるかを記録している。
...
- 接続ごとに、スコープ内の User のあるべき状態と、下流へ反映済みの状態を突き合わせ、差分に対して `ProvisioningTask` を作る照合を実装し、定期ジョブとして走らせる。
...
- `docs/domain/provisioning/internals.md` の「同一トランザクションでのプロビジョニングタスクの記録」と、`docs/domain/structure.md` のライフサイクルの通知の記述を現状に合わせる。

## 対象外

- 捕捉が抜けている各経路（LifecycleWorkflow、SCIM の取り込み、CSV インポート、プロフィール変更）への個別の捕捉の追加。照合が回収するので、遅延が問題になる経路が分かってから別の項目で扱う。
- Group のプロビジョニングタスクの照合。まず User で照合の形を確かめてから、同じ形で広げる。
...
反映済みの状態は、`RemoteResourceLink` と、その User について最後に確定したプロビジョニングタスクの `source_version` から読む。

| あるべき状態 | 反映済みの状態 | 作るプロビジョニングタスク |
| --- | --- | --- |
| スコープ内で有効 | リンクなし | `create` |
| スコープ内 | リンクあり、User の版が最後に確定したプロビジョニングタスクより新しい | `update`、または状態に応じた無効化や再有効化 |
...
管理者が手動で走らせる Full Resync（`REQ-PROVISIONING-013`、`usecases.StartFullResync`）は、スコープ内の全対象へ `update` のプロビジョニングタスクを一律に作る。
照合との違いは次の 2 点である。

- 照合は変わっていない対象を飛ばし、差分だけにプロビジョニングタスクを作る。
...
照合が作るプロビジョニングタスクも、既存の冪等キー `(tenant, connection, source_type, source_id, source_version)` を使う。
`source_version` には User の版を使い、書き込み時の捕捉と照合が同じ変更に対して同じ版を使うようにする。
これで、書き込み時の捕捉が成功した変更を照合が二重にプロビジョニングタスクすることはない。
...
- 記録の正の変更は、有効な接続を持つ下流へ、照合の周期以内にプロビジョニングタスクされる。
- 書き込み時の捕捉の失敗や、発火元の経路が捕捉を呼ばないことは、この保証を破らない。

具体例は次の形へ改める。

- `EX-PLATFORM-003-01`: 変更が書き込み時に捕捉されれば、プロビジョニングタスクが作られて `succeeded` になる。
- `EX-PLATFORM-003-02`: 書き込み時の捕捉が失敗しても、次の照合でプロビジョニングタスクが作られる。
- 新しい具体例: 捕捉を呼ばない経路で状態が変わっても、次の照合でプロビジョニングタスクが作られる。
...
2. 「最後に確定したプロビジョニングタスクの版」をプロビジョニングタスクの行から都度求めるか、`RemoteResourceLink` に最後に反映した版を持たせるか。
...
- [ ] T002 [Acceptance] 捕捉を呼ばない経路で User を無効化してもプロビジョニングタスクが作られないことを RED として観測する。
- [ ] T003 [App] 照合の計算（あるべき状態と反映済みの状態から作るプロビジョニングタスクの集合）を純粋な関数として実装する。
...
- **照合が大量のプロビジョニングタスクを一度に作る。** 初回の照合や長い停止の後は差分が大きい。誤削除ガードの閾値と、1 回に作るプロビジョニングタスクの上限で抑える。
- **照合と書き込み時の捕捉が同じ変更を二重にプロビジョニングタスクする。** `source_version` を揃えて冪等キーで重複を落とす。揃えるまでは二重プロビジョニングタスクが起こり得るので、T004 を照合の有効化より先に行う。
