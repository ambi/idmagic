...
- [x] T001 [SCL] standards.SAML2Core.SAML2Core-BearerAssertion、standards.SAML2Bindings.SAML2Bindings-RedirectPost、standards.SAML2WebBrowserSSO.SAML2Profile-WebBrowserSSO、models.SamlServiceProvider / SamlAuthnRequest、interfaces.SamlSingleSignOn、scenarios.SAML SP initiated SSO succeeds を更新し、時間・passive・binding・ACS・NameID・replay 要件、失敗 scenario、SAML protocol error outcome を追加して `just yaml-check` / `just scl-render` を GREEN にした。
- [x] T002 [Domain] RED: `TestParseAuthnRequestRejectsMissingVersionOrIssueInstant` と `TestValidateSignInAtRejectsUnsupportedAuthnRequestSemantics` を追加し、未実装 field / validator で fail 確認（models.SamlAuthnRequest）→ Version / IssueInstant / skew、IsPassive、ProtocolBinding、ACS URL/index、NameID format の fail-closed validator を GREEN にした。
- [x] T003 [Port/Adapter] RED: memory replay store の逐次・TTL・tenant isolation・並行 `RecordIfNew` test を追加し、未実装 store で fail 確認（interfaces.SamlSingleSignOn ensure）→ mutex memory と SET NX TTL Valkey adapter を GREEN にした。
- [x] T004 [Usecase/HTTP] passive 未認証をログイン遷移せず検証済み ACS への assertion-less NoPassive HTTP-POST response にし、未対応 semantic と replay は assertion 発行前に fail-closed とした。既存の正常 login resume / ForceAuthn / HTTP SSO contract test を GREEN にした。
- [x] T005 [Verify] `just format-go`、`just test-go`、`just yaml-check`、`just scl-render` を実行した。
...
- **Summary**:
  AuthnRequest の Version / IssueInstant / binding / ACS / NameID semantics を fail-closed に検証し、
  passive の未認証要求には検証済み ACS へ NoPassive response を返すようにした。tenant / SP /
  request ID の TTL replay reservation を assertion 発行直前に置き、memory と Valkey の両方で
  原子的に一度だけ発行できるようにした。
- **Affected Guarantees State**:
  未対応または古い AuthnRequest は成功扱いされず、同じ SP-issued request による assertion の
  再発行は拒否される。replay store の障害も fail-closed となる。
- **Verification Results**:
  - `just format-go` / `just test-go` — passed
  - `just yaml-check` / `just scl-render` — passed
- **対応していないこと (ADR-121 の開示義務)**:
  - SAML ECP、encrypted assertion、inbound federation、ACS index の解決、XML parser fuzzing /
    corpus 運用は対象外のままである。
