...
- `docs/domain/*.yaml` の全 19 ファイル・75+ 個の `kind: error` model へ
  `status` を付与。
...
1. `SPECIFICATION_CORE_LANGUAGE.md` と `tools/check/schemas/scl-v3.schema.json`
   ...
   ことを確認する (既存 `docs/domain/*.yaml` は `status` 未設定のまま
...
3. `docs/domain/*.yaml` を 1 ファイルずつ (または関連するまとまり単位で)
...
- [x] T004 [SCL] `docs/domain/*.yaml` の全 `kind: error` model へ
