...
# 高 sensitivity な self-service 操作に step-up 再認証を要求する
...
## Scope
- **decision**:
  - 新規 ADR-043 **account-portal-csrf-and-step-up**: 高 sensitivity な mutation の 一覧と、step-up の条件 ("直近 N 分以内に password or MFA で再認証済み"、N=5 分) を 定める。step-up 未通過は 401 ではなく **403 + `step_up_required`** で返し、UI が 再認証 modal を出す。対象表は SCL interface annotation で機械照合する。
- **scl**:
  - 新規 interface: StartStepUpAuthentication / CompleteStepUpAuthentication。 新規イベント: StepUpRequested / StepUpCompleted。sensitive interface に step-up 要求のアノテーションを付ける。
- **go**:
  - AuthenticationContext に "最後に factor を提示した時刻" を持たせ (既存 auth_time / セッションの amr 履歴を利用)、step-up gate の usecase を追加する。 gate は handler 共通ミドルウェアとして sensitive ハンドラに適用する。
- **http**:
  - `/api/account/step_up/start` / `/api/account/step_up/complete`。既存の sensitive ハンドラ (change password / MFA remove / email change / revoke_others) に gate を 差し込み、未通過時は 403 + `step_up_required`。
- **ui**:
  - sensitive 操作で 403 `step_up_required` を受けたら再認証 modal を出し、成功後に 元の操作を再試行する共通フックを SPA に追加する。
- **documentation**:
  - README の account portal 節に step-up の対象操作と挙動を明記する。
...
## Verification
- `go test ./...` (in: idmagic)
  - reason: step-up gate (5 分超過で 403) / 再認証後の通過 / 対象表と実装の一致。
...
- **Affected Guarantees State**:
  - CSRF & step-up: 対象表の sensitive mutation は step-up を必須化し、未通過は 403 + `step_up_required`。対象表 (ADR-043) と実ハンドラの一致を SCL の `step_up: required` 注記で機械照合 (テストで網羅)。維持。
  - self RBAC: step-up は actor 本人の再認証で、対象 sub を変えない (session の step_up_at に刻むため別端末の cookie でも当該 session に閉じる)。維持。
  - SCL coherence: StartStepUpAuthentication / CompleteStepUpAuthentication interface、 StepUpRequested / StepUpCompleted event、StepUp* model を SCL に追加し coherence_test と yaml-check を通過。維持。
