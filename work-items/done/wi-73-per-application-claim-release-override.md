...
- [x] T005 [Admin UI] `AdminApplicationEditPage.tsx` に OIDC 向け claim release editor を追加。`AdminApplicationsShared.tsx` に `ClaimReleaseAttributesPreview` (非PII preview: `getTenantUserAttributeSchema` から key/type/visibility を一覧、visibility=private を強調表示、実ユーザー値は表示しない) を新設し OIDC/WS-Fed/SAML の3 editor に共通で埋め込む。`AdminApplicationDetailPage.tsx` に OIDC の sub_source_attribute/rules 表示を追加。バックエンド側で `handleUpdateOIDCConfig`/`handleUpdateWsFedConfig`/`handleUpdateSamlConfig` に `ValidateClaimReleaseRules` (write-time floor 検証、400 拒否) と `ApplicationClaimMappingUpdated` イベント発行を実装。RED: `claim_release_test.go` の `TestUpdateApplicationOidcConfig_RejectsUndefinedAttributeSource` 等を先に fail 確認 (OIDC は decode エラー、WsFed/Saml は204で無検証通過) → GREEN。`bun run typecheck/lint/test:unit/build` green。
  - **追記 (同日フォローアップ)**: 初回実装は WS-Fed/SAML の既存踏襲で 3 protocol とも生 JSON textarea だったが、ユーザーレビューで「なぜ JSON を書かせるのか、sub ソース属性は選択肢を絞るべきでは」と指摘を受け、構造化エディタに置き換えた。`AdminApplicationsShared.tsx` に `useReleasableAttributes` (テナント属性 + User の core field を1回だけ取得し floor 準拠の選択肢を返す)、`SourceAttributeSelect` (NameID/sub source をドロップダウンで選択、floor 対象外の値は選べない)、`ClaimRulesEditor` (claim_type 入力・source 選択・source_key ドロップダウン/fixed_value 入力・required チェックボックスを行単位で編集、生 JSON を書かせない) を追加し、OIDC/WS-Fed/SAML の3 editor 全てをこれに置換した。state も JSON 文字列から `WsFedClaimMappingRule[]` 配列に変更し、JSON.parse の例外処理と `invalidClaimRulesJsonError` を削除した。sub のデフォルト値は `backend/claimmapping/usecases/floor.go` の `coreAttributeKeys` に含まれ常に releasable なため妥当と確認済み (`sub` → `ResolveUserAttributes` が `u.ID` を割り当てる、既存 ID Token/UserInfo の `sub` 挙動と同じ)。architecture.yaml の complexity ratchet (`ui-page-lines` for AdminApplicationDetailPage.tsx を 410→413、`ui-page-local-state` for AdminApplicationEditPage.tsx を 34→36) を明示的に引き上げ、reason に本 WI を追記。`bun run typecheck/lint/test:unit/build` と `just check` すべて green。
  - 開示: ブラウザでの実 UI 目視確認は本セッションでは実施できなかった (ブラウザツール未接続)。`just dev-memory` を起動し API 経路の生存確認は行ったが、ログイン画面からの実クリック操作は未検証。バックエンドの新規挙動 (rules roundtrip・fail-closed 拒否・ID Token/UserInfo マージ) は Go の HTTP 統合テスト (`claim_release_test.go` 等、実サーバーコード経路) で検証済み。構造化エディタ自体も実ブラウザでの目視確認は未実施。
