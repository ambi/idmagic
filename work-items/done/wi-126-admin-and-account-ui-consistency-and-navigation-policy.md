...
# 管理コンソール／マイページの UI 一貫性・ナビゲーションポリシー整備とユーザー詳細遷移バグ修正
...
- **decision**:
  - 本 WI の横断ポリシー (詳細→編集ナビゲーション / 一覧行アクションの見せ方 /
    ...
    ([[wi-39-admin-detail-pages]] / [[wi-37-admin-ui-consistency-and-localization-polish]] /
    [[wi-16-admin-shell-visual-consistency]]) を踏まえ、重複しない粒度で残す。
- **documentation**:
...
- 監査イベント基盤そのものの機能追加 (新しいイベント種別・検索軸など)。本 WI は
  ダッシュボードでの見せ方と用語統一に閉じる ([[wi-44-audit-event-store-and-search]] /
  [[wi-46-authentication-event-attribute-emit-and-correlation-search]] の範囲は触らない)。
- admin 詳細画面そのものの新設 (すでに [[wi-39-admin-detail-pages]] で完了)。
  本 WI は「詳細→編集」遷移ポリシーの徹底と未追随画面の是正に閉じる。
- 新規バックエンドエンドポイント・集約の追加。バグ修正はフロントの sub→id 追随のみ。
- 認証・権限境界・テナント分離の変更。
- ログイン画面の視覚デザイン刷新 ([[wi-89-tenant-login-branding]] の範囲)。本 WI は
...
- 内部表現の非露出: ラベルや監査イベント表示で内部識別子・内部イベント名を
  生で出さない ([[wi-19-rich-user-attributes]] / [[wi-37-admin-ui-consistency-and-localization-polish]] 継続)。
...
## 2026-07-05 — 残りの全セクションおよびポリシー文書化完了 (UI のみ)

残っていたすべての課題（ユーザー登録/グループの専用画面化、マイページ導線・URL分離、ダッシュボード再設計、用語統一、ADR化、UIガイドライン記述）を完了。

- **§6 ユーザー登録の専用画面化**:
  - `AdminUsersPage.tsx` の `CreateUserDialog` モーダルを専用画面 `AdminUserCreatePage` へリファクタ。
  - ルート `/admin/users/new` (実体: `ui/src/routes/admin/users_/new.tsx`) を新設。
  - 一覧画面の「ユーザーを追加」ボタンをこの新規ルートへのリンクに変更。
- **§6 グループ追加・編集の専用画面化**:
  - `AdminGroupsPage.tsx` の `GroupEditorDialog` モーダルを専用画面 `AdminGroupCreatePage` と `AdminGroupEditPage` へリファクタ。
  - ルート `/admin/groups/new` (`ui/src/routes/admin/groups_/new.tsx`), `/admin/groups/$groupId/edit` (`ui/src/routes/admin/groups_/$groupId.edit.tsx`) を新設。
  - `/admin/groups/$groupId` 詳細表示用のルートを `$groupId.index.tsx` へ分離し、`$groupId.tsx` は Outlet レイアウトルートにリネーム。
  - 一覧画面および詳細画面の編集/追加ボタン、右ペインの `AdminPaneActions` で `editHref` へのリンクに切り替え。
- **§9 マイページ「アカウント情報」の導線・URL**:
  - アカウントプロフィール詳細と編集を分離し、`/account/profile/edit` (`ui/src/routes/account/profile_/edit.tsx`) を新設。`profile.tsx` は Outlet レイアウトルートに変更。
  - プロフィール詳細画面にメール編集画面（`/account/emails`）への「変更する」導線を追加。
- **§4 ダッシュボード再設計**:
  - クイックリンクから「署名鍵を確認」を外し、実務的な「サインインポリシーを設定」を追加。
  - 直近の監査イベントの型名（`RefreshTokenIssued` 等）を生で表示するのをやめ、日本語の分かりやすいイベント表記に変換する関数 `friendlyEventName` を導入。
  - イベントクリック時に直接関連リソース詳細（`/admin/users/{id}` や `/admin/applications/{id}`）へ飛べるリンク遷移関数 `eventLink` を導入。
- **用語統一 (監査ログ → 監査イベント)**:
  - `AdminAuditEventsPage.tsx`, `adminNav.ts`, ダッシュボード内の表記を「監査イベント」へ統一。
- **ADR / Documentation**:
  - `decisions/ADR-086-ui-navigation-consistency-and-page-title-policy.md` にて本 UI 一貫性ポリシーを ADR 化。
  - `ui/ARCHITECTURE.md` に `## UI navigation and consistency policy` セクションを追加し、今後の UI 画面追加でもこのポリシーが適用されるよう明文化。
- **検証**: `just verify` (全Goテスト、Linter、UIビルド・フォーマット) すべて green。

## Completion
- **Completed At**: 2026-07-05
- **Summary**:
  管理コンソールとマイページの UI 構成および遷移ポリシー（詳細→編集分離）を適用し、モーダルから専用のルート画面へと変更。ダイナミックページタイトル表示、監査イベント名の日本語フレンドリー表示化と関連リソースへの直接リンク遷移の実装、および「監査イベント」への用語統一を実施。ADR-086の策定と ui/ARCHITECTURE.md へのポリシー明文化を完了。
- **Verification Results**:
  - `just verify` (format/lint/typecheck/build/test) すべて passed.
