...
initial_context:
  ...
  typespec:
    ...
    - IdMagic.Contract.InvalidTargetError
  source:
...
行ごとの観測の形は `Adoption` が決める。`required` は宣言した振る舞いが正式な入口から到達できること、`optional` は提供しているならその振る舞い、`excluded` は提供していないことと拒否が防いだ効果、`partial` は採った範囲と採らなかった範囲の扱いを、それぞれ観測する。

**本項目の 7 行はすべて `required` である。** 着手前に `standards.md` の当該 4 節を読んで確かめた。したがって `optional` と `excluded` の観測の型を決める判断は本項目には現れない。Plan の 2 と 3、および Tasks の T003 はこの理由で消える。

## Plan

1. 7 行が共有する入口にハーネスを組み、`required` の 1 件目を通しで消化して型を決める。
2. ~~`optional` があれば、その 1 件目で「提供している」と言える根拠の形を決める。~~ 7 行に `optional` は無い。
3. ~~`excluded` があれば、その 1 件目で観測の型を決める。~~ 7 行に `excluded` は無い。
...
- [x] T002 [Harness] `/.well-known` の各文書と JWKS の配信経路 にハーネスを組み、`required` の 1 件目で型を決める。
- [x] T003 [Type] ~~`optional` と `excluded` の観測の型を、それぞれ 1 件目で決める。~~ 7 行はすべて `required` なので、決める判断が無い。
...
- [x] T006 [Defect] ~~宣言した採用を満たしていない行が見つかったら、欠陥の work item を切り出す。~~ 7 行とも満たされていた。切り出しは無い。
...
- **Summary**:
  ...
  規範の宣言と製品コードは変えていない。
  宣言済みのメタデータ配信 7 行は、正式な HTTP 入口から宣言内容と実際の振る舞いを対にして観測できるようになり、標準の被覆台帳から外れた。
  宣言した採用を満たしていない行は見つからなかった。

- **Acceptance RED Evidence**:
  - **Test**: `mise run check-spec`
  - **Requirement**: N/A: 宣言済みの標準行へ検証を対応付ける保守であり、製品の規範要求は変更しない。
  - **Observed Failure**: 台帳から 7 ID を外し、テストの注記を無効にした状態では、`RFC7517-JWKS`、`RFC8414-METADATA`、`RFC9728-METADATA`、`RFC9728-WELL-KNOWN`、`RFC9728-IDMAGIC-API`、`RFC9728-CHALLENGE`、`OIDC-DISCOVERY-CONFIGURATION` のそれぞれについて `is declared, but no test names it` を報告して exit 1 になった。
  - **Detection Reason**: 検査は標準行、テストから見つけた ID、被覆台帳を独立に突き合わせるため、テストを足さずに台帳だけを削除した状態を 7 行ごとに区別する。
...
- **Change-Resistance Results**:
  production の判断を 1 つずつ崩し、対応するテストが assertion まで到達して落ちることを確かめた。

  - `RFC7517-JWKS`：配信する JWK から `n` と `e` を除くと、`TestJWKSPublishesOnlyPublicVerificationKeys` が `JWK に n が無い` で落ちた。
  - `RFC8414-METADATA`：Authorization Server Metadata の `issuer` を変えると、`TestAuthorizationServerMetadataPublishesTheIssuerEndpointsAndCapabilities` が広告値と realm の発行者識別子の不一致で落ちた。
  - `OIDC-DISCOVERY-CONFIGURATION`：`userinfo_endpoint` のフィールド名を変えると、`TestOpenIDProviderConfigurationPublishesTheIssuerEndpointsAndCapabilities` がフィールドの欠落で落ちた。
  - `RFC9728-WELL-KNOWN`：handler が `resource` ではなく `aud` を読むようにすると、`TestProtectedResourceMetadataIsFetchedFromTheWellKnownPathByResource` が対象リソースの不一致と別名の誤受理で落ちた。
  - `RFC9728-METADATA`：登録済みリソースの `scopes_supported` を空にすると、`TestProtectedResourceMetadataIsDerivedFromEachRegisteredResourceServer` が配列の欠落で落ちた。
  - `RFC9728-IDMAGIC-API`：realm API の `bearer_methods_supported` を空にすると、`TestProtectedResourceMetadataWithoutAResourceDescribesTheRealmApi` が配列の欠落で落ちた。
  - `RFC9728-CHALLENGE`：challenge の `resource_metadata` パラメーター名を変えると、`TestBearerChallengesPointAtTheRealmProtectedResourceMetadata` の `invalid_token` と `insufficient_scope` が両方とも URL の欠落で落ちた。

- **Verification Results**:
  - 7 件を指定した `mise run test-go-test` - passed
  - `mise run test-go-package -- ./backend/shared/http/server_http` - passed
  - `mise run lint-go` - passed (`0 issues`)
  - `mise run check-spec` - passed (`156 standard(s)`, `264 id(s) named by a test`)
