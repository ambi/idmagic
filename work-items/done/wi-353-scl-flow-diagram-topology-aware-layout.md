...
これは `spec/scl.yaml`・`docs/domain/*.yaml`・`tools/check/schemas/scl-v3.schema.json`
...
- `spec/scl.yaml` / `docs/domain/*.yaml` の `flows` セクションの記法変更
...
- **Summary**:
  ...
  SCLスキーマ(`spec/scl.yaml`/`docs/domain/*.yaml`/`scl-v3.schema.json`)は
