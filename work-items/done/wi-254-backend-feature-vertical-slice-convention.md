...
- [x] T006 [Verify] 下記 Verification を実行し全緑を確認。
...
- **Summary**:
  ...
  の 3 feature へパイロット変換した。`domain`(feature 固有型)/`ports`/`usecases`/
  `adapters/persistence/memory` の4層を `git mv` で再配置し、import path を一括置換した。
  パイロットの過程で当初計画になかった2つの技術的制約が判明し、ADR-130 に決定として明記した:
  (1) `adapters/http` は Go の「メソッドは receiver 型と同一パッケージにしか定義できない」
  制約により、`Deps` 構造体とそのハンドラメソッド一式を feature 分割できないため
  context ルート共有のまま維持した。(2) `adapters/persistence/postgres` は sqlc が
  1 つの `queries/` から 1 つの `sqlcgen` パッケージを生成する制約と、postgres テストの
  fixture ヘルパー(`seedUser`/`seedGroup` 等)が feature 横断で共有されている制約により、
  同じく context ルート共有のまま維持した。domain 層では `enums.go`/`events.go` 等
  feature 横断の型のみ共有 `domain/` に残し、usecases 層では `normalizeRoles` 等の
  feature 非依存ヘルパーと `ErrUserNotFound`/`ErrInvalidRole` 等 feature 横断エラー変数を
  新設 `backend/idmanagement/usecases/helpers.go`(共有)に集約した。`ARCHITECTURE.md` の
  module ledger を feature 粒度(`idmanagement-{user,group,agent}-{domain,ports,usecases}`、
  memory persistence 用の `-adapters`)へ再設計し、約 30 の外部 module の `depends_on` を
  実際の import 先へ更新した。
- **Out of Scope として明示的に対応していないこと** (ADR-121 開示):
  - `adapters/http` と `adapters/persistence/postgres` の feature 分割は、上記の技術的
    制約により本 wi では実施していない。`Deps` を feature ごとの embedded 部分構造体へ
    再設計する、または sqlc の多パッケージ生成へ移行するといった、より踏み込んだ改修は
    ADR-130 の「却下した代替案」として記録し、必要になれば別 work item で扱う。
...
- **Verification Results**:
  ...
  - `just test-go` — passed(idmanagement の全 feature サブパッケージのテストを含む)。
  ...
  - `git mv` による履歴保持は commit 後に `git log --follow` で確認する。
  - 旧配置(`backend/idmanagement/domain|ports|usecases` の feature 固有シンボル)への
    import 残存はゼロ(grep で確認。共有パッケージ `backend/idmanagement/domain`・
    `backend/idmanagement/usecases` への参照は意図的に残存)。
