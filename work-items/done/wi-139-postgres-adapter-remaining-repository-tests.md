---
id: wi-139-postgres-adapter-remaining-repository-tests
title: postgres adapter の未テスト・リポジトリに対する統合テストを追加する
...
- **Defect Found & Fixed (application_orderings の tenant_id 排除)**:
  ...
  原因は idp-ADR-083（globally unique client_id / user_id、composite FK 廃止）の適用漏れ:
  ...
  - Docs: `postgres.sql` 冒頭の tenant_id key policy コメントを ADR-083 反映へ更新。
    idp-ADR-083 に `user_id` 従属子の `tenant_id` 排除方針（`application_orderings` 明記）を追記。
