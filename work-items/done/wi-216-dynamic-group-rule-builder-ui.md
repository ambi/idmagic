...
- **Summary**:
  動的グループの CEL ルールを属性・演算子・値から組み立てるビルダー UI を追加し、
  生成した式を既存の preview / save / enable フローへ接続した。
...
SCL の動的グループルール契約、既存の CEL コンパイル・評価・preview / save / enable
API 契約は維持され、UI から生成する CEL 式だけが追加された状態である。

- **Verification Results**:
  - `just yaml-check-work-items` — passed on 2026-07-24 after canonical dependency references were restored
