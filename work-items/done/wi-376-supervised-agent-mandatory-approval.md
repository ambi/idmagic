...
- `docs/domain/oauth2/scenarios.feature.md` に、`Supervised` な Agent へのトークン発行が人間の承認を経ることを求める規範シナリオを追加する。対象は Agent が主体となるすべての発行経路 — `client_credentials`、トークン交換、ワークロード ID 連携による交換 — とする。
