---
id: idp-wi-24-hibp-breached-password-checker
title: "HIBP k-anonymity による漏洩パスワード検査 adapter を投入する"
...
「攻撃者がどの password を試すか」のうち静的な事前知識しか弾けない。
実運用では「過去に他サイトで漏洩した password」を当てるクレデンシャル
スタッフィング攻撃が主流であり、辞書だけでは届かない。

ADR-028 (breached password checker) は port + HIBP Range API
(k-anonymity) を採用すると既に決めているが、実装は
`NoopBreachedPasswordChecker` のみで、本番 adapter (`hibp`) と
`BREACHED_PASSWORD_CHECKER` 環境変数の bootstrap 配線が未だ無い。
...
- HIBP 以外の漏洩 DB (SpyCloud / FlareID 等) 連携。port が同じなので 将来 adapter を増やすのは容易だが本 WI ではしない。
- 過去に保存済の password に対する遡及検査。本 WI が影響するのは 本 WI 投入後に新しく set される password のみ。
- fail-open の判定を tenant policy で fail-close に倒すオプション。 本 WI は global fail-open 固定。可用性を最優先する判断 (ADR-028 §3)。
...
主リスクは 3 点:
  ...
  (1) 外部依存追加: HIBP への egress を許す環境でしか hibp は使えない。
      proxy 環境では `HTTP_PROXY` を尊重する std net/http 設定で済む
      はずだが、proxy 経由でも timeout 2 秒に収まるか確認する。

  (2) fail-open の運用上ノイズ: 障害時に password が通ってしまう。
      metric `breached_password_checker_failures_total` を SLO 監視に
      積む前提で運用する (本 WI で metric 出力までは入れる。alert 化は
      Phase 8)。

  (3) HIBP API のレート / 利用規約: 個人で叩く分には rate limit に
      達しないが、本番運用では User-Agent と利用目的を明示する。
      本 WI で `User-Agent: idmagic/<version>` を固定する。

辞書 + history + HIBP の 3 段で覆える範囲が運用最低ラインで、本 WI
でその最低ラインに到達する。

# Completion
- **Completed At**: 2026-06-28
- **Summary**:
  ADR-028 の実装側を完成させた。`internal/infrastructure/policy/`
  (既存 Noop adapter と同じ package) に HibpBreachedPasswordChecker を新設し、
  SHA-1 prefix 5 文字を HIBP Range API に GET する k-anonymity 検査、
  Add-Padding ヘッダ、2 秒 timeout、`User-Agent: idmagic/<version>`、
  prefix 単位の TTL 付き LRU cache (TTL 10 分 / 上限 1024) を実装した。
  HTTP error / timeout / 5xx / 接続失敗は false を返す fail-open とし、
  失敗は warn log と OpenTelemetry counter
  `breached_password_checker_failures_total{reason}` で可視化する。
  bootstrap に `BREACHED_PASSWORD_CHECKER` (`noop` 既定 / `hibp`) を読む
  resolveBreachedPasswordChecker を追加し、起動 log を 1 行出して Deps に
  注入する。usecase 層 (reset_password_with_token) は無変更で本 adapter を通る。
