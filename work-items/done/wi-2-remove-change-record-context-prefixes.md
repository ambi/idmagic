...
# Work Item と ADR のコンテキストプレフィクスを廃止する
...
- **Summary**:
  ...
  追記 (2026-07-09): 「既存 ID を履歴互換のため据え置く」という上記判断は、work-item/ADR フォーマット移行の一環でユーザーの明示的な指示により上書きされた。既存の `idp-wi-*` / `repo-wi-*` / `idp-ADR-*` も含め全件を無プレフィクスの単一名前空間へ一括リネームし、リポジトリ内の相互参照も併せて更新した。
