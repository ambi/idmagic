...
# scl.yaml から wi / ADR / commit 参照を除去し純粋な仕様文にする
...
## Scope
- **scl**:
  - scl.yaml 内の `(wi-NN)` / `(ADR-NNN)` / `(commit <hash>)` / `[[wi-...]]` / `[[ADR-...]]` 参照を除去し、残る文を自然な仕様文に整える。意味は保持する。
- **verification_only**:
  - 仕様の意味不変を yaml-check と既存テストで担保する。
...
- **Summary**:
  ...
  全体 (spec/scl.yaml + docs/domain/*.yaml) と解釈して適用した。除去前は 6 ファイル
...
- **Verification Results**:
  ...
  - `GOCACHE=/tmp/idmagic-cache go test ./...` (in: idmagic)
    - result: ok (全 pass)
  - 手動: `grep -rnE 'wi-[0-9]|ADR-[0-9]|commit [0-9a-f]{7}' spec/scl.yaml docs/domain/*.yaml` が 0 件、整形崩れ (空括弧・行頭句点・区切り残り) も 0 件であることを確認した。
