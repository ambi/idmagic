...
completion:
  ...
  summary: "McpResourceServer の tenant-scoped 管理 UI、型付き API client、管理ナビと route を追加し、実ブラウザ相当 E2E で一覧・登録・編集・削除の往復を確認した。"
  verification:
    - "just verify-ui"
    - "just verify"
    - "SEED_ENVIRONMENT=development SEED_PROFILE=development just test-ui-e2e"
  ...
  evidence:
    - id: "frontend-unit-contract"
      kind: "test"
      procedure: "API client contract、英日表示、空状態、scope 変換の unit/component tests"
      result: "passed; 74 test files and 409 tests"
    - id: "frontend-verification"
      kind: "static_analysis"
      procedure: "UI format check, lint, typecheck, unit tests, and production build"
      result: "passed"
    - id: "repository-verification"
      kind: "test"
      procedure: "SCL/Work Item/Architecture validation, Go verification, and UI verification"
      result: "passed"
    - id: "mcp-resource-server-browser-lifecycle"
      kind: "runtime_observation"
      procedure: "Bun.WebView で admin route へログインし、McpResourceServer の登録・編集・Disabled 化・削除を実 API 経由で往復"
      result: "passed; UI action suite 14/14, smoke 3/3, session recovery 1/1, OIDC golden path 2/2"
---
