---
id: idp-wi-112-persistence-mode-rename-postgres-valkey
title: "PERSISTENCE モードの値を postgres から postgres_valkey に改名し durable/volatile 双方のバックエンドを名前へ反映する"
...
- **docs**: README (env 表・HA 節)、ARCHITECTURE、deploy/docker/docker-compose.dev.yaml、decisions/idp-ADR-016 (選択子の定義と改名理由) を更新する。

# Out of Scope
- durable/volatile を独立の環境変数へ分離する2軸化 (現状 2 択のため将来課題)。
- 過去の履歴記録 (idp-ADR-024 / idp-ADR-077、work-items/done/*) 内の `PERSISTENCE=postgres` 表記の遡及修正。その時点の記録として据え置く。
