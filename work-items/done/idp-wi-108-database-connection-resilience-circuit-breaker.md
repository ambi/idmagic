...
- `ResilientDB` ラッパーによる PostgreSQL 接続へのタイムアウトとサーキットブレイカー、および `resilience.RetryWithBackoff` による接続時の Exponential Backoff リトライの実装。
- 各リポジトリ（20ファイル）の `pgxpool.Pool` 直接参照を `DB` インターフェースに変更し、ラップされた `ResilientDB` を DI。
- Valkey クライアントに `redis.Hook` を用いた透過的なサーキットブレイカーおよびコンテキストタイムアウトのインジェクションを実装。
- JWKS および Discovery メタデータの公開エンドポイントにおいて `sync.Map` を用いたインメモリキャッシュでのグレイスフル劣化（フォールバック）を実装。
- すべての単体テスト、ビルド、`golangci-lint` をパスしていることを確認（`just verify-go` を完遂）。
