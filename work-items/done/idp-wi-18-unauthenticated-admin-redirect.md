---
id: idp-wi-18-unauthenticated-admin-redirect
title: "未認証で /admin/* を開いたとき login に redirect し、戻り先を保持する"
...
UI 側の `request()` が AuthenticationAPIError を投げて
「認証を続行できません」というエラー文言で画面が止まる。
これは UX として明確に悪い:

1. ログインへの動線が無く、admin はブックマークを開き直すか
   URL を `/login` に手で書き換える必要がある。
2. ログイン後に元のページ (`/admin/users` 等) に戻る経路が
   失われ、もう一度 sidebar を辿り直すことになる。
3. エラー文言が「認証サービスに接続できませんでした」と読めるため、
   サービス障害と取り違える可能性がある (実態は単に未ログイン)。

業界標準 (Okta / Google Admin / Microsoft 365) は未認証時:
  - 自動的に login にリダイレクト
  - 元の URL を query / state に乗せて、ログイン後に deep-link
    復帰させる
を提供する。本 WI はこの 2 点を満たす。
...
- 認証済だが admin role が無い user が `/admin/*` を開いたときの 挙動。これは既に 403 access_denied として正しく動いているため、 別 WI で「権限不足ですメッセージ + sidebar 一括無効化」の UI 改善が必要なら扱う。
- ログイン後に多段の戻り先 (consent → /admin/users) を持つケース。 idmagic が IdP 自身として admin 用 client を持っているかどうか 現状で判別する仕組みを足すのは本 WI のスコープを越える。
- エラー画面の汎用化 (404 / 500 / network エラーの統一画面)。 別 polish WI でまとめる。
...
- 手動 4: `returnTo=http://attacker/x` 等の外部 URL を強引に sessionStorage / query に入れた状態でログインしても、外部 URL には飛ばず `/admin` (or default) に戻る。
- 手動 5: admin role 無し user で `/admin/users` を開いた場合は 本 WI の対象外。既存の 403 挙動が変わっていないことを確認。

# Risk Notes
open redirect は実装ミスで脆弱性になるため、`returnTo` の検証は
ホワイトリスト方式を厳格に守る。テストで攻撃ベクタを 2-3 個
(`//attacker/`, `/\\\\attacker/`, `%2F%2Fattacker/`,
`https://attacker/`) を加えて検証する。

ログイン経路として `/authorize` を選ぶか `/login` を選ぶかは、
idmagic 自身が IdP であることに由来する判断。`/authorize` を選ぶ
場合は管理 SPA 用の OAuth client (例: `admin-spa`) が常に存在する
ことを前提にする。default tenant では `demo-client` を流用しても
よいが、本 WI で client metadata を変更する必要は無いか確認する。

# Completion
- **Completed At**: 2026-06-16
- **Summary**:
  `/admin/*` の初期ロードで account API が 401 を返した場合、現在の path と
  query を `return_to` として `/login` へ移動する。ログイン API と TOTP API
