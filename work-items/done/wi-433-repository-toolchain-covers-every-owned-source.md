...
- **MD001 (見出しの飛び)。** 23 件のうち 22 件は `docs/contexts/*/scenarios.feature.md` の `# 見出し` → `### REQ-...` である。[SPECIFICATION_FORMAT.md](../../SPECIFICATION_FORMAT.md) 6 節が「シナリオの見出しは、上に何も無くても H3 のままにする。`### REQ-...` は参照、検査、生成されるアンカーがすべて前提にしている形である」と明文で定めている。規範どおりに書かれた正準文書 22 本を、違反として報告し続ける。
