...
## Tasks
- [x] T001 [Domain] action の状態判定ロジックを副作用なしの評価関数として抽出する。
- [x] T002 [App] dry-run usecase を `enabled_revision` 対象・実状態評価に置き換える。
- [x] T003 [HTTP] handler の決め打ち `would_change` を削除し usecase の結果を返す。
- [x] T004 [Verify] action 種別ごと、trigger 不一致、revision 分岐を table test で検証する。
...
- **Summary**: `idmdomain.EvaluateWorkflowAction` を副作用なしの評価関数として抽出し、action
  executor (`executeLifecycleAction`) と新しい `DryRunLifecycleWorkflow` usecase の両方から呼ぶ形に
  リファクタリングした。dry-run は `enabled_revision` (未 enable なら `current_revision`) を対象に、
  対象 User の group membership / application assignment / required action / status / email 検証
  状態を実際に読んで `would_change` / `no_op` / `blocked` を判定する。trigger の filter が対象 User
  に一致しない場合は全 action を `blocked` (reason: `trigger_not_matched`) として返す。副次的に、
  executor 側の `unassign_application` が既存の割当有無を確認せず常に `changed` を返していた不整合も
  同じ評価関数で解消した (dry-run と実行結果の食い違いを防ぐため)。
- **Affected Guarantees State**: dry-run は引き続き WorkflowRun / Job / membership / assignment /
  required action / status / email を一切作成・変更しない。既存の step 実行・retry・冪等性の挙動は
  回帰確認済みで変化なし。
- **Verification Results**:
  - `just yaml-check` — passed
  - `just verify-go` (lint + `go test -race ./...`) — passed
  - `just verify` (yaml-check, verify-go, verify-ui) — passed
  - 自動: `TestEvaluateWorkflowAction`、`TestDryRunLifecycleWorkflowNoOpWhenUserAlreadyAtGoalState`、
    `TestAdminLifecycleWorkflowDryRunReflectsActualUserState` が「既に group member である User への
    add_group_member dry-run は no_op」を検証する。
  - 自動: `TestDryRunLifecycleWorkflowUsesEnabledRevisionNotDraft` が enable 済み revision と別の
    draft 編集がある状態で dry-run が draft を無視し enabled_revision を評価することを検証する。
  - 自動: `TestDryRunLifecycleWorkflowBlockedWhenTriggerFiltersDoNotMatch` が trigger filter 不一致
    時に全 action が `blocked`/`trigger_not_matched` になることを検証する。
- **Evidence**: ローカル開発環境で Claude Code が実装・テスト追加・`just verify` 実行まで行った。
