...
initial_context:
  specification: [docs/SPECIFICATION.md, docs/domain/identity-management/SPECIFICATION.md]
...
現状 400 (`http.StatusBadRequest`) で specification は 422 (`http.StatusUnprocessableEntity`)
...
現状 400 で specification は **409** (状態競合) を宣言している code
...
- [x] T001 [App] `admin_user_handler.go` を移行し、`invalid_role`・
      ...
      RED→GREEN: `TestAdminUserAPIRejectsSelfDelete`・
      `TestAdminUserAPISetsAndClearsRequiredAction`。
- [x] T002 [App] `admin_group_handler.go` を移行し、`invalid_role`・
      `group_name_required`・`invalid_dynamic_group_rule` を 422 に揃える。
      `InvalidEmailError`/`InvalidGroupAttributeError` も `CreateGroupError422`/
      `UpdateGroupError422` の要素なので `invalid_email`・`invalid_attribute` も
      同時に 422 へ揃えた。
      RED→GREEN: `TestAdminGroupAPICreateRejectsInvalidEmail`・
      `TestAdminGroupAPICreateRejectsUndefinedAttribute`・`TestIdentityAPIErrors`。
- [x] T003 [App] `admin_agent_handler.go` を移行し、`invalid_role`・
      `agent_name_required`・`agent_owner_required`・`agent_owner_not_found` を
      422 に揃える。RED→GREEN: `TestIdentityAPIErrors`。
- [x] T004 [App] `email_change_handler.go` を移行し、`invalid_email`・
      `email_unchanged` を 422 に揃える。RED→GREEN: `TestIdentityAPIErrors`。
- [x] T005 [App] `admin_data_export_handler.go` を移行し、`invalid_columns`・
      `invalid_target`・`invalid_filter` を 422 に揃える。
      RED→GREEN: `TestDataExportHTTP_RejectsInvalidColumns`。
- [x] T006 [App] `account_handler.go`・`admin_user_import_handler.go` を
...
`admin_agent_handler.go` の 3 ファイルすべてに出現する同名 code。specification 側は
...
- **Summary**:
  `backend/idmanagement` 配下 7 ファイル・81 箇所の `WriteBrowserError` を
  すべて `support.WriteProblem` へ置き換えた (起票時の箇所数からいくつかの
  ファイルで増えていたが、ファイル構成は同じ)。
  status を仕様の宣言値へ揃えたのは 19 箇所:
  - 422: `admin_user_handler.go` の `invalid_role`・`self_delete_forbidden`・
    `self_disable_forbidden`・`invalid_attribute`・`invalid_required_action`、
    `admin_group_handler.go` の `invalid_role`・`group_name_required`・
    `invalid_dynamic_group_rule`・`invalid_email`・`invalid_attribute`、
    `admin_agent_handler.go` の `invalid_role`・`agent_name_required`・
    `agent_owner_required`・`agent_owner_not_found`、
    `email_change_handler.go` の `invalid_email`・`email_unchanged`、
    `admin_data_export_handler.go` の `invalid_columns`・`invalid_target`・
    `invalid_filter`。
  - 409: `admin_user_handler.go` の `not_pending_deletion`・
    `restore_grace_expired` (`RestoreAdminUserError409`)。
  起票時の一覧になかった `admin_group_handler.go` の `invalid_email`・
  `invalid_attribute` も、`CreateGroupError422`/`UpdateGroupError422` が
  `InvalidEmailError`/`InvalidGroupAttributeError` を持つため同時に揃えた。
...
- **Verification Results**:
  - `just test-go-package ./backend/idmanagement/...` - RED
    ...
    が旧 status/envelope で失敗) → GREEN
  - `just test-go` - passed (全パッケージ。`TestIdentityAPIErrors`・
    `TestAdminGroupAPICreateRejects*`・`TestAdminUserAPISetsAndClearsRequiredAction`
    の status 期待値も 422 へ更新した)
  - `just verify` - passed
