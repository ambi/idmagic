...
- [x] T001 [Design/Spec] ADR-057 の supported events/subject format/SET profile を確定し、revocation objective、stream/delivery lifecycle、events/scenarios を再生成する。
  ...
  - `docs/domain/oauth2/SPECIFICATION.md` の `Introspect` に revocation epoch 判定の `ensures` とシナリオ「kill-switch後のAgentトークンはintrospectionでactive=falseになる」を追加。
  - `docs/domain/api-tokens/SPECIFICATION.md` に `shared-signals:read`/`shared-signals:write` scope を追加。
...
- [x] T002 [Domain/Persistence] AgentRevocationEpoch、SSFStream、SecurityEventDelivery を実装し、memory/PostgreSQL の tenant-scoped store を追加する。
  ...
  - `docs/domain/sharedsignals/SPECIFICATION.md` を新規追加し、root `docs/SPECIFICATION.md`/`docs/SPECIFICATION.md` の `backend/sharedsignals` (planned) を実装済みへ更新。
...
- [x] T003 [Enforcement] KillAgent/credential unbind/owner offboard と epoch 更新を接続し、OAuth2 Introspect で評価する。
  ...
  - design 同期: `docs/domain/sharedsignals/SPECIFICATION.md` に `sharedsignals-usecases` module を追加し、bootstrap/worker/http-server と OAuth2 の依存を実 import に合わせて `docs/SPECIFICATION.md` へ反映する。
...
- [x] T004 [Transmitter] SET builder/signer、outbox projector、retry/backoff/dead-letter delivery を実装する。
  ...
  - design 同期: `docs/domain/sharedsignals/SPECIFICATION.md` に `sharedsignals-usecases`（更新）/`sharedsignals-push-http`/`sharedsignals-sign-jose` を整理し、bootstrap/worker/http-server と OAuth2 の依存を `docs/SPECIFICATION.md` へ反映する。
...
- [x] T005 [Admin] stream CRUD/key metadata、delivery health を追加する (kill 結果表示は delivery health API に統合)。
  ...
  - design 同期: `docs/domain/sharedsignals/SPECIFICATION.md` に `sharedsignals-adapters` (handlers_http) と `tenancy-public` 依存を反映する。
...
- [x] T007 [Receiver] (新規、当初の Task 内訳に無かった欠落を今回追加) ReceiveSecurityEvent (inbound SET 受理) を実装する。
  ...
  - design 同期: `docs/domain/sharedsignals/SPECIFICATION.md` に `sharedsignals-verify-jose` と `idmanagement-agent-ports` 依存を反映する。
...
- **Summary**:
  ...
  `docs/domain/sharedsignals/SPECIFICATION.md` と `backend/sharedsignals/` (domain / ports /
