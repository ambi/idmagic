...
- **開発ルールの明文化**:
  - `docs/development/specification-first-workflow.md` の Verification ladder に、カバレッジ閾値を段の 1 つとして書く。
...
- **Summary**:
  取り消す。現在の検証方針は一律の行カバレッジ率を品質の代理指標にせず、規範上の具体例、主要ユースケースの RED、故障注入によって検出能力を確かめる。全体率と差分率の閾値を必須化する本項目は現行方針と両立せず、価値の低いテストを増やすため、実装せずに終了する。
