...
## Tasks
- [ ] T001 [SCL] application/claim-mapping/oauth2/saml/ws-federation に ClaimReleasePolicy、composition order、interfaces/events/constraints/contracts/scenarios を追加して再生成する。
- [ ] T002 [Domain] baseline/override/effective policy evaluator、reserved claim registry、attribute visibility check を実装する。
- [ ] T003 [Migration] 既存 SAML SP/WS-Fed RP claim policy と OIDC scope mapping を inventory し、Application binding から effective policy を解決する互換層を追加する。
- [ ] T004 [Protocol] OIDC ID Token/UserInfo、SAML assertion、WS-Fed token の各 issuer を共通 effective result へ接続する。
- [ ] T005 [Admin UI] Application editor に protocol別出力、source/visibility、validation付き override と非PII preview を追加する。
- [ ] T006 [Verify] baseline deny、reserved claim、scope/consent、rename collision、多値属性、3 protocol の同等 policy、migration roundtrip を検証する。
