...
- **decision**:
  - `docs/domain/tenancy/decisions.md` へ記録する決定: `TenantDomain` の所有権検証方式 (DNS TXT)、`Pending` を予約にしない
...
- **apex ドメインを拒否する** (`login.example.com` は可、`example.com` は不可)。
  ...
  IdP に向ける事故を防ぐ。
- **トライアル抑止は既存 quota 機構に乗せる**。`docs/domain/tenancy/scenarios.feature.md` の `TenantQuota` の resource に `custom_domains` を
...
- [ ] T002 [Spec] 所有権検証方式・`Pending` 非予約・apex 拒否・quota・一意制約の範囲を
      `docs/domain/tenancy/decisions.md` に記録する。
