...
親状態は開始時の対象集合と終端結果を持ち、ジョブハンドラーがプロビジョニングタスクを終端化した後に更新する。
完了判定とイベント発行は同じ永続化境界で一度だけ成立させる。

## 計画

1. プロビジョニングタスク・ジョブ・イベントの既存永続化境界を調査する。
2. 全対象の登録と、最後の成功または失敗が完了を確定する RED を作る。
3. 親状態、プロビジョニングタスクへの関連付け、終端時の集計とイベント発行を実装する。
4. 結合経路と変異を検証する。
...
- [ ] T004 [Use Cases] 開始・プロビジョニングタスク処理・イベント発行を接続する。
