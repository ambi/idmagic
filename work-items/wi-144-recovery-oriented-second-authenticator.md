...
復旧経路が絶たれる（`docs/domain/authentication/decisions.md` の層状のアカウント復旧）。
...
- `docs/domain/authentication/` (`scenarios.feature.md`、`decisions.md`、`models.tsp`):
...
- recovery code の設計変更（`docs/domain/authentication/decisions.md` の hash-only / single-use / 全置換は維持）。
