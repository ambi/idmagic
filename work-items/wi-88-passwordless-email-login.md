...
## Scope
- **decision**:
  - `docs/domain/authentication/decisions.md` へ記録する決定: email magic link / email OTP を passwordless first-factor として 採用する。challenge は期限付き・単発消費 (`docs/domain/authentication/decisions.md` のワンタイムトークン方針)、 amr / acr へ反映 (例 amr: otp / mfa)、account enumeration 抑止、テナント設定 (allow_passwordless_email) でのゲートを記録する。
- **specification**:
