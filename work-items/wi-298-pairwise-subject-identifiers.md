...
- **decision**:
  - `docs/domain/oauth2/decisions.md` へ記録する決定 (subject identifier の写像): `public` を既定に維持する理由、`pairwise` の
...
- **specification**:
  ...
  - `ClaimMapping` に subject 写像の位置付けを追記する
    (`docs/domain/claim-mapping/decisions.md` の claim 発行境界と整合)。
...
- **salt は不変にする**。salt をローテーションすると全 RP でユーザーが「別人」になり、
  ...
  `docs/domain/oauth2/decisions.md` に「ローテーション不可」を明記する。鍵ローテーション
  (`docs/domain/signing-keys/decisions.md` の鍵ローテーション方針) の対象外であることも書く。
...
- **切り替えの不可逆性を UI で示す**。public → pairwise の変更は RP 側のユーザー紐付けを
  壊す。設定変更時に警告を出し、`docs/domain/oauth2/decisions.md` にも影響として書く。
...
- 未決定: sector identifier を `sector_identifier_uri` で持つか、Application 単位にするか。
  この repo は Application を単一の編集面とする方針 (`docs/domain/application/decisions.md`)
  なので、**Application を sector の単位とする**のが最も整合する。第一候補とし、
  OIDC 準拠のため `sector_identifier_uri` を将来の拡張余地として `docs/domain/oauth2/decisions.md` に残す。
...
- [ ] T002 [Spec] subject identifier の写像の決定を `docs/domain/oauth2/decisions.md` に記録する (算出方式・sector の単位・
...
`docs/domain/oauth2/decisions.md` にローテーション不可を明記し、切り替えの不可逆性を UI で警告する。
