...
- [ ] T001 [Persistence] audit event の postgres/memory 実装を
  `oauth2/adapters/persistence/{postgres,memory}` へ同居。
- [ ] T002 [Persistence] outbox の postgres 実装を
  `oauth2/adapters/persistence/postgres` へ同居。
- [ ] T003 [Persistence] audit event / outbox の postgres 実装を sqlc 生成へ
  置換（動的検索はエスケープハッチ）。
- [ ] T004 [DI] `oauth2/module.go` に `AuditEventRepo`/`EventSink` を統合。
- [ ] T005 [DI] 中央 `server/routes.go` `Deps` と `bootstrap/deps.go` から
  oauth2 関連フィールドを完全撤去。
- [ ] T006 [Measure] audit event 検索の動的クエリ比率を実測し [[ADR-090]] に
  追記。
- [ ] T007 [Verify] `just verify-go` / `just test-go` green、oauth2 の
