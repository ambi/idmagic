...
- 旧配備の版付き `DeploymentMetadata` と新成果物の `BuildMetadata` を入力に、`EvaluateUpdateCompatibility(previous, next)` が `rolling`、`recreate`、`invalid` のいずれかと理由コードを返す。
...
- `rolling` 判定は後続の異版混在受け入れテストを通過した版の組合せだけに許可する。
...
判定は決定表として実装する。すべての変更が `rolling` なら `rolling`、一つでも再作成を要求すれば `recreate`、入力矛盾または未対応の移行があれば `invalid` を返す。理由は機械可読な閉じたコードとして複数返し、人向け説明を別に生成する。

終了コードは `0=rolling`、`10=recreate`、`20=invalid`、`30=unexpected_failure` とし、内部 JSON の項目追加では自動化を壊さない。終了コードの割当は正準運用文書に記録し、後方互換な追加以外は版を上げる。

## Plan

1. 配備前判定のシナリオ、入力、出力、失敗時の配備停止を仕様化する。
2. metadata schema と決定表を Unit RED から実装する。
3. 現在の成果物と配備設定から秘密を含まない metadata を生成する。
4. `mise` タスク、CI、リリース手順へ読み取り専用の gate として接続する。
5. 異版混在テストの結果を `rolling` 判定の証拠へ結び付ける。

## Tasks

- [ ] T001 [Spec] 更新互換性の判定、終了コード、フェイルクローズ、配備停止を正準シナリオと運用文書へ定める。
- [ ] T002 [Domain] metadata 型と `EvaluateUpdateCompatibility` を代表的な決定表の Unit RED から実装する。
- [ ] T003 [Build] 新成果物と旧配備から秘密を含まない版付き metadata を生成および取得する。
- [ ] T004 [CLI] 安定した終了コードと要約を返す読み取り専用コマンドを `mise` タスクで公開する。
- [ ] T005 [Release] CI とリリース手順で `invalid` と予期しない失敗を配備前に拒否する。
- [ ] T006 [Acceptance] 直前安定版、機能版変更、停止必須設定、未対応 metadata を実成果物境界で検査する。
...
- `mise run verify`
- `rolling`、`recreate`、`invalid`、予期しない失敗がそれぞれ安定した終了コードを返す。
- metadata とログに秘密値が含まれず、未知の版や欠落入力が `rolling` へ倒れない。

## Risk Notes

リスクは high。誤った `rolling` 判定は配備中の停止やデータ破損へ直結するため、未知の入力を再作成可能と推測せず `invalid` にする。判定器は配備を実行せず、実際の異版混在検査と rollback 手順を別の証拠として要求する。
