...
- **decision**:
  - `docs/domain/oauth2/decisions.md` へ記録する決定 (署名付き認可メッセージ): JAR の受け入れ方式 (`request` パラメータのみ対応し、
    外部 `request_uri` fetch は SSRF 面のため**対応しない**方針を明記する)、署名アルゴリズムの
    許可集合 (`docs/domain/oauth2/standards.md` の `RFC7518-SIGNATURE-ALGORITHMS` と整合し PS256 / ES256 に限定)、
    ...
    レスポンス署名鍵 (`docs/domain/signing-keys/decisions.md` のテナントごとの署名鍵 の tenant key を使う)、
...
- **go**:
  - request object の検証を `backend/oauth2` に実装する。クライアントの登録済み JWKS
    ...
    `docs/domain/oauth2/internals.md` の `private_key_jwt` 検証 の検証基盤を再利用する。
  - `jti` のリプレイ防止ストア (短命 / PostgreSQL、`docs/design/data/database.md` の一時状態の PostgreSQL 統合
    に従う) を追加する。
...
- **ui**:
  - 管理コンソールのアプリケーション編集 (OIDC 詳細設定) に
    ...
    (`docs/domain/application/decisions.md` に従い Application 配下に置く)。
- **documentation**:
...
- **`request_uri` を非対応にするのが安全側の設計判断**である。外部 URL を取得する実装は
  ...
  `request_uri` を得る」という等価な体験は既に提供できている。`docs/domain/oauth2/decisions.md` にこの等価性を書く。
- **検証は既存の private_key_jwt 検証基盤に載せる**。`docs/domain/oauth2/internals.md` の `private_key_jwt` 検証 で
...
- **リプレイ防止は `jti` + `exp` で行う**。`jti` ストアの保持期間は `exp` の最大許容値に
  ...
  (`docs/domain/audit/decisions.md` の保持期間 の retention sweep) に載せる。
...
- [ ] T002 [Spec] 署名付き認可メッセージの決定を `docs/domain/oauth2/decisions.md` に記録する (request_uri 非対応の理由・
