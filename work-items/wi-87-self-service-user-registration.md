...
## Scope
- **decision**:
  - `docs/domain/authentication/decisions.md` へ記録する決定: self-registration をテナント設定 (allow_self_registration、既定 off) でゲートする。email 検証必須 (`docs/domain/authentication/decisions.md` のワンタイムトークン方針を踏襲)、 tenant 内 email 一意、登録直後は未検証 = login 不可 (fail-closed)、 account enumeration を避ける応答方針を記録する。
- **specification**:
