...
- **specification**:
  ...
  - `docs/domain/legacy-access/` に `README.md`、`glossary.md`、`states.md`、`decisions.md`、`scenarios.feature.md`、
    `models.tsp`、`main.tsp` を追加する。権限境界は `docs/design/security/authorization.md` に合流させる。
  - `docs/domain/application/` に `legacy_form` protocol binding と、代理認証を使う
    Application のポータル起動情報を追加する。
- **decision**:
  - `docs/domain/application/decisions.md` に、標準フェデレーションを優先する境界、秘密情報の保管/発行境界、
...
- **architecture / documentation**:
  - `docs/README.md` と `docs/domain/structure.md` に LegacyAccess context と依存方向を同期する。
...
- [ ] T003 [Spec] 代理認証の安全境界と運用例外を `docs/domain/application/decisions.md` に記録する。
