...
change_kind: bugfix
affected_spec:
  - { path: docs/domain/standards.md, requirement: GDPR-PROCESSING-RECORDS }
---
...
`docs/domain/audit/decisions.md` は「監査レコードは GDPR 第 30 条 (処理活動の記録) を根拠に、追記のみで 7 年間保持する。削除やアーカイブのインターフェースは提供しない」と決めている。
...
- 決めた側に合わせて、`docs/domain/audit/decisions.md` か `RetentionPolicy` と `DeleteOlderThan` の位置づけのどちらかを変える。
...
- 監査イベントの検索属性と PII の変換方針。`docs/domain/audit/internals.md` が持つ。
...
- **決めずに実装だけ揃えてしまう。** 7 年と 365 日は運用費用も法的な意味も違う。どちらが正しいかを先に決め、根拠を `docs/domain/audit/decisions.md` に残してから片側を動かす。
...
- `docs/domain/audit/decisions.md` の保持の決定と、`RetentionPolicy` の既定および `DeleteOlderThan` の位置づけが一致している。
