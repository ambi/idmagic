...
- 39 件を 1 件ずつ確認し、次のいずれかに解決して `tools/check/example-coverage-debt.json` から外す。
  - 当の具体例を検証しているテストが実在する → そのテストに `// EX-TENANCY-NNN-MM: <この具体例の何を固定しているか>` の注記を足す。
...
- `mise run check-spec` が、`docs/contexts/tenancy/scenarios.feature.md` の 39 件を `tools/check/example-coverage-debt.json` から外した状態で通る。台帳から外す前に同じ検査が当の id を名指しで落とすことを、消化ごとに観測する。
