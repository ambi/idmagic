...
([[ADR-094-transactional-event-log-and-audit-projection]])。しかし現在の監査ログは
...
- **decision**:
  - 新規 ADR (監査ログ完全性): ハッシュ対象フィールドの正規化規則 (フィールド順序・時刻表現・
    NULL 表現)、チェーンの単位 (テナント単位で独立)、チェックポイント発行間隔と署名鍵
    ([[ADR-075-per-tenant-signing-keys-and-key-provider]] の鍵を使うか専用用途鍵を切るか)、
    保持期間による古いイベント削除 ([[ADR-045-authentication-event-retention]] /
    [[ADR-107-audit-retention-and-jobs-dev-environment-topology]]) とチェーン継続の両立方式
...
- 認証イベント以外のアプリケーションログの完全性
  ([[ADR-018-audit-vs-application-log-separation]] により別物として扱う)。
...
- **正規化を最初に固定する**。ハッシュ対象の直列化が曖昧だと、後の実装変更で過去の
  ...
  NULL 表現を ADR に固定し、正規化関数の golden test を最初に書く。
...
- 未決定: チェックポイント署名鍵を既存 tenant signing key と共用するか専用用途にするかは
  ADR で決める。監査用途の鍵をトークン署名と共用すると鍵ローテーションの制約が絡むため、
  用途分離を第一候補とする。
...
ADR に固定し golden test で守る。将来の変更は「バージョン付き正規化」で扱う余地を残す。
