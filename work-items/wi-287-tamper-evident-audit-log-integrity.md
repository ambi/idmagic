...
(`docs/domain/audit/decisions.md`)。しかし現在の監査ログは
...
- **decision**:
  - `docs/domain/audit/decisions.md` へ記録する決定 (監査ログ完全性): ハッシュ対象フィールドの正規化規則 (フィールド順序・時刻表現・
    NULL 表現)、チェーンの単位 (テナント単位で独立)、チェックポイント発行間隔と署名鍵
    (`docs/domain/signing-keys/decisions.md` のテナントごとの署名鍵 の鍵を使うか専用用途鍵を切るか)、
    保持期間による古いイベント削除 (`docs/domain/authentication/decisions.md` の認証イベント保持期間 /
    `docs/domain/audit/decisions.md` の保持期間) とチェーン継続の両立方式
...
- 認証イベント以外のアプリケーションログの完全性
  (`docs/domain/audit/decisions.md` により別物として扱う)。
...
- **正規化を最初に固定する**。ハッシュ対象の直列化が曖昧だと、後の実装変更で過去の
  ...
  NULL 表現を `docs/domain/audit/decisions.md` に固定し、正規化関数の golden test を最初に書く。
...
- 未決定: チェックポイント署名鍵を既存 tenant signing key と共用するか専用用途にするかは
  `docs/domain/audit/decisions.md` で決める。監査用途の鍵をトークン署名と共用すると鍵ローテーションの制約が絡むため、
  用途分離を第一候補とする。
...
- [ ] T002 [Spec] 監査ログ完全性の決定を `docs/domain/audit/decisions.md` に記録する (正規化規則・チェーン単位・チェックポイント
...
`docs/domain/audit/decisions.md` に固定し golden test で守る。将来の変更は「バージョン付き正規化」で扱う余地を残す。
