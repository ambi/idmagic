...
initial_context:
  scl:
    Audit:
      ...
      - interfaces.ExportAdminAuditEvents
      - interfaces.GetAdminAuditEventSearchOptions
    Jobs:
      - interfaces.EnqueueJob
    ApiTokens:
      - interfaces.IssueApiToken
  decisions:
    - decisions/ADR-018-audit-vs-application-log-separation.md
    - decisions/ADR-098-durable-job-queue-skip-locked-lease.md
    - decisions/ADR-120-event-relay-transport-abstraction-and-pubsub.md
...
現状、監査イベントは (1) 管理 API で検索・CSV エクスポートできる、(2) `EVENT_SINK=outbox`
で Kafka にリレーされる、の 2 経路しかない。前者は pull 型で人間向け、後者は
**デプロイ全体で 1 本の内部トランスポート**であり、テナントが自分のイベントだけを自分の
宛先へ購読する手段が無い。
...
- **decision**:
  - 新規 ADR (outbound event hook と配送保証): 配送セマンティクス (at-least-once、順序保証は
    しない、重複は購読側が `event_id` で冪等化)、HMAC 署名方式とヘッダ仕様、再試行と指数
    バックオフ、連続失敗時の自動無効化 (circuit) 条件、SSRF 対策 (宛先の URL 検証と private
    IP 範囲の拒否)、payload に載せる PII の範囲 ([[ADR-046-authentication-event-pii-policy]] と
...
- **scl**:
  ...
  - `objectives` に配送捕捉レイテンシ (イベント発生から delivery 行の作成まで) の目標を追加する
    ([[ADR-128-extract-provisioning-context-and-transactional-delivery-capture]] の
...
- **transactional capture を先に決める**。「イベントは発生したが配送行が作られていない」状態を
  ...
  [[ADR-128-extract-provisioning-context-and-transactional-delivery-capture]] が
  Provisioning で既に同じ形を採っているので、その構造を踏襲し新しい発明をしない。
- **配送は既存 Jobs 基盤に載せる**。retry / lease / 進捗可視化は
  [[ADR-098-durable-job-queue-skip-locked-lease]] と
  [[ADR-099-job-worker-execution-model-and-fault-tolerance]] が既に提供しているので、
...
- **PII は既定で最小**。payload には event_id / type / 時刻 / tenant / actor 識別子 /
  ...
  詳細が必要な購読者は管理 API で引く前提にする ([[ADR-046-authentication-event-pii-policy]])。
...
payload の PII 過多は情報漏洩面を広げる。既定を最小にし、拡張は明示的な ADR 更新を要する。
