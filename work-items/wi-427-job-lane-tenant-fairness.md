...
これは[脅威モデル](../docs/design/security/threat-model.md)の THREAT-022（テナント別の偏り）と THREAT-072（下流の障害による枠の占有）が指す欠落である。どちらも同じ機構の欠落から来る。
...
取得規則には複数の案がある。テナントごとの同時実行上限を設ける案、テナントを巡回して取得する案、滞留しているテナントの取得確率を下げる案である。いずれも `FOR UPDATE SKIP LOCKED` の単純さを損なうため、どこまで複雑にするかは、想定する偏りの大きさで決まる。`docs/design/performance/capacity.md` の参照運用プロファイルはテナント規模の分布を持っているので、それを入力にできる。
...
2. 想定する偏りを `docs/design/performance/capacity.md` の分布から導く。
3. 取得規則を決め、判断として書く。
...
公平性の規則は取得の問い合わせを複雑にし、`FOR UPDATE SKIP LOCKED` の性能特性を変えうる。`docs/design/performance/capacity.md` のジョブレーンの枠算出が前提としている処理率を下回らないことを確認する。
