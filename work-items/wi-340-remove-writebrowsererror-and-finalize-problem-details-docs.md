...
# `WriteBrowserError` を削除し Problem Details 移行完了を ARCHITECTURE.md に記録する
...
envelope を返す `WriteBrowserError` 自体を削除し、`ARCHITECTURE.md` の
...
- `backend/shared/http/support_http/response.go` の `WriteBrowserError`
  関数本体の削除。
- `ARCHITECTURE.md` の HTTP error responses 節
  (「この規約は未実装」段落) の更新。
...
4. `ARCHITECTURE.md` の HTTP error responses 節を更新する。
5. `just verify` を通す。
...
- [ ] T003 [Docs] `ARCHITECTURE.md` の HTTP error responses 節を実装完了
      状態に更新する。
