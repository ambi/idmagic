...
initial_context:
  scl:
    System:
      ...
      - scenarios.EnvironmentSpecificSeedDataIsSelected
      - scenarios.RepeatedSeedApplyIsNoOp
      - scenarios.SeedDryRunDoesNotMutate
      - scenarios.UnsafeProductionDemoSeedIsRejected
  decisions: [ADR-084]
  source: [backend/cmd/internal/bootstrap, backend/cmd/idmagic]
  tests: [backend/cmd/internal/bootstrap]
  stop_before_reading: [frontend]
affected_spec:
  - { context: System, kind: interface, element: StartupProbe }
  - { context: System, kind: scenario, element: "orchestration probeはprocess lifecycleと依存状態を区別する" }
---
...
## Scope
- **SCL**:
  - `spec/contexts/system.yaml` の `glossary` に `SeedProfile` を追加する。
...
## Tasks
- [ ] T001 [Inventory/ADR] 現行 seed 全件と repository semantics を棚卸しし、bootstrap/demo 境界、profile manifest、stable ID、drift、排他、checkpoint、secret、ライブラリ不採用方針を ADR に記録する。
- [ ] T002 [SCL] `SeedProfile`、`SeedRequest`、`SeedPlan`、`SeedData`、`SeedScalability` と正常・境界・失敗・拒否 scenario を `spec/contexts/system.yaml` に追加する。
- [ ] T003 [Render] `just scl-render` で SCL 派生物を同期する。
- [ ] T004 [Core] profile parser / validator、deterministic generator、semantic planner、redacted streaming result、件数上限を実装する。
- [ ] T005 [Apply] bounded batch、依存順、同時実行 lock、部分失敗から再実行可能な apply orchestration を port / use case 経由で実装する。
- [ ] T006 [Idempotency] password/history/timestamp/membership を含む全 seed 対象を、同一入力の再実行では永続状態を変更しないよう修正する。
- [ ] T007 [CLI/Startup] 独立 seed command と `just` recipe を追加し、server の暗黙 demo seed、`just dev` の明示 profile、production guard、`SKIP_DEMO_SEED` 移行を整備する。
- [ ] T008 [Verify] memory/PostgreSQL で dry-run、連続・並行 apply、drift、partial failure、profile 分離、上限、stream/batch の contract・integration・race test を追加する。
- [ ] T009 [Scale/Docs] wi-161 と共有する大規模 profile の opt-in 計測 recipe、README の profile matrix・安全手順・secret/PII 非出力説明を追加する。
