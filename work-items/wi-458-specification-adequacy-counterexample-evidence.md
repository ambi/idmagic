...
- 値ベースの property testing とメタモルフィック関係（`wi-420`）、設計時のモデル検査と決定的シミュレーション（`wi-421`）、リスクに応じた故障注入または変異の証拠（`wi-437`）。いずれも実装とテストの側から仕様を検証する手段であり、本項目は仕様そのものの未決定を対象とする。全リポジトリ共通の変異試験基盤を作る `wi-457` は取り消されており、本項目の前提にしない。反例を性質へ一般化する作業は `wi-420` へ渡す。
