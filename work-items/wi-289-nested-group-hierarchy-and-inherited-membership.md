...
initial_context:
  scl:
    IdManagement:
      - models.Group
      - interfaces.CreateGroup
      - interfaces.UpdateGroup
      - interfaces.DeleteGroup
      - interfaces.ListGroups
      - interfaces.ListUserGroups
      - interfaces.AddGroupMember
  decisions:
    - decisions/ADR-038-group-aggregate-and-effective-roles.md
    - decisions/ADR-111-cel-dynamic-group-membership-rules.md
  source:
...
[[ADR-038-group-aggregate-and-effective-roles]] もフラットな union を前提としている。
...
- **Okta**: グループ階層を持たない (Okta は代わりに group rule で解く) — つまり
  ...
  IdMagic は動的グループ ([[ADR-111-cel-dynamic-group-membership-rules]]) を持つが、
...
- **decision**:
  - 新規 ADR (グループ階層と継承の意味): 単一親 (森構造) に限定する理由、循環禁止と最大深さ、
    継承の方向 (子グループのメンバーは親グループの継承メンバーである、の向きを 1 つに固定する)、
    `effective_roles` の再定義 (直接ロール ∪ 所属グループとその祖先のロール)、
    dynamic group ([[ADR-111-cel-dynamic-group-membership-rules]]) と階層の併用可否、
    ApplicationAssignment の解決に継承メンバーシップを含めるか
    ([[ADR-064-protocol-contexts-and-application-catalog]] の fail-closed 割当と整合)、
...
- **go**:
  ...
  - 継承メンバーシップの解決は **closure table** (`group_closure(ancestor_id, descendant_id, depth)`)
    ...
    書き込み時に閉包を更新する。
  - ApplicationAssignment の解決に継承メンバーシップを含める (ADR の決定に従う)。
- **persistence**:
...
割当ゲートを緩める形になるため、ADR に意味を明記し、「親に割当てると子の全員に効く」ことを
UI でも明示する。
