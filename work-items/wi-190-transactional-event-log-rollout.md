...
- [ ] T001 [App] OAuth2 の単純 admin CRUD（client/consent）を transaction runner へ移行する。
- [ ] T002 [App] application context の CRUD を transaction runner へ移行する。
- [ ] T003 [Decision/App] `refresh_tokens.go` の transaction 設計を ADR で決定し、
  OAuth2 の複雑プロトコルフローを移行する。
- [ ] T004 [Decision/App] SAML/WS-Fed の admin SP/RP CRUD への emit 新設要否を決定し、
  必要なら実装する。
- [ ] T005 [App] SCIM の実際の emit 呼び出し箇所を確認し、migration 方式を実装する。
- [ ] T006 [Relay] relay を `event_deliveries` に対応させ、at-least-once、stable event ID、
  failure recording をテストする。
- [ ] T007 [Arch/Verify] `ARCHITECTURE.md`、README の runtime 説明を同期し、回帰・障害注入・
  relay 再実行検証を完了する。
