...
1. **配置先を決め、接頭辞を選ぶ**（§2.1 / §1.1.1）
   - 対象アプリ / コンテキストの近くの `decisions/` に置く（例 `apps/foo/decisions/`、接頭辞 `foo`）。
     手法・横断レベルの決定はルートの `decisions/`（接頭辞 `repo`）。
2. **採番する**
   - そのコンテキストの既存を確認: `ls <app>/decisions 2>/dev/null`
   - ファイル名は `decisions/<接頭辞>-ADR-NNN-kebab-title.md`。`NNN` はコンテキスト内の 3 桁ゼロ詰め連番。
   - **連番は再利用しない。廃止した ADR も削除せず残す**（過去の決定経緯は再生成の文脈になる）。
   - 並行採番で被っても `just check-ids` が検出するので、被ったら採り直す（§1.1.2）。
...
5. 廃止時は「ステータス」に置き換え先 ADR と日付を添える（例 `廃止（idp-ADR-015 に置き換えられた、2026-03-01）`）。

## スケルトン

```markdown
# repo-ADR-NNN: 一文で表す決定
