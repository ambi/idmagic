---
name: scl-change
description: SCL-first workflow for feature work. Use before implementing any feature or behavior change — update the spec (scl.yaml) first, decide which SCL sections to touch, validate, and handle derived artifacts according to branch mode. Use when adding/changing a feature, endpoint, model, state, permission, or non-functional target.
---
...
5. **派生物の扱いをブランチの役割で決める。** `scl-render` Skill 参照。
   - 単独開発、integration branch、main 直前では `just scl-render` で HTML / JSON Schema /
     OpenAPI を再生成し、SCL と派生物を同期させる。
   - 並列 worktree の work-item branch では、衝突を減らすため派生物の commit は原則しない。
     必要なら確認用に `just scl-render` を実行してよいが、生成物の差分は統合地点で作り直す。

6. **構成が変わるなら Architecture を同期する。** 新しい境界づけられたコンテキストの追加や、
   モジュールが実現する SCL 要素の増減など、`ARCHITECTURE.md` の地図に映る構造変化を伴うときは、
   `new-architecture` Skill で該当 `ARCHITECTURE.md` を更新する（純粋な仕様の追加・修正だけなら不要）。

7. 実装はこの後。完了したらワークアイテムに `completion` を追記する（`new-work-item` Skill §1.3）。
