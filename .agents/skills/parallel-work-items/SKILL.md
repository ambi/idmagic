...
```sh
git fetch --all --prune
git worktree add -b work-item/idp-wi-42-example ../idmagic-wi-42 <base-branch>
```

既存 branch を別 worktree に出す例:

```sh
git worktree add ../idmagic-wi-42 work-item/idp-wi-42-example
```
...
Implement work-items/idp-wi-42-example.md end to end, verify it, update completion,
...
git merge --no-ff work-item/idp-wi-42-example
git merge --no-ff work-item/idp-wi-43-example
```
