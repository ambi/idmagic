...
3. Change the specification first with the `spec-change` Skill, and pass `mise run check-spec`.
...
6. Update each task as it completes. Run the narrowest test recipe that covers what you touched, then
   pass `mise run verify`.
7. Record the semantic difference and the verification results in `Completion` — `mise run spec-diff`
