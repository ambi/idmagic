...
export type ApplicationIconFileValidationError =
  | 'too-large'
  | 'unsupported-type'
  | 'invalid-signature'
...
export async function validateApplicationIconFile(
  ...
  if (file.size > MAX_APPLICATION_ICON_BYTES) return 'too-large'
  if (!isApplicationIconFile(file)) return 'unsupported-type'
  try {
    if (!(await hasApplicationIconSignature(file))) return 'invalid-signature'
  } catch {
...
async function hasApplicationIconSignature(file: File): Promise<boolean> {
