...
独立したADR台帳は持たない。現在有効な判断は、それを所有するファイルに、判断・理由・見直し条件として置く。変更時点の比較検討はwork itemに残す。
...
| コンテキストが所有する概念と所有しない概念 | そのコンテキストの`README.md` |
...
そのコンテキストが**何を所有し、何を所有せず、所有しないものを誰が代わりに持つか**を書く。所属の判断を誤りやすい場合は、それを決める基準も書く。
...
User、Group、Accountのライフサイクルと、それらに付随するメタデータを所有する。
資格情報とセッションは所有しない。Authenticationが持つ。
...
| `README.md`の様子 | 対処 |
|---|---|
| 一文で所有物を言える。機能が同じ語彙と不変条件を共有する | 機能へ分ける |
| 所有物を言うのに「および」が要る。機能どうしが違う語彙を使う | コンテキストを割る |
| 所有物は言えるが、他コンテキストの語を頻繁に持ち込む | 境界を引き直す |
...
複数のコンテキストが使う部品は、所有者を決めてその所有者が詳細を持つ。ここには索引だけを置く。
...
アダプターは所有するコンテキストの直下に置き、`<role>_<technology>`で命名する。
...
| 保持期間 | `standards.md`のGDPRの行と、所有するコンテキストの`decisions.md` |
| 型の選び方、制約の置き方 | この文書 |
...
`models.tsp`がモデルの宣言を、`main.tsp`が操作を持つ。各操作は所有するコンテキストのタグを継承させる。所有者が移っても、外部に見えている名前は変えない。
...
障害の事後記録は、対策が恒久的な規則になった時点でその規則を所有する仕様へ反映する。記録そのものの形式は本ガイドで定めない。
