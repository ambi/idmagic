...
│   │   └── scenarios.md
│   │
│   ├── development/              # 手順。環境構築、ビルド、生成、CI、テスト、リリースと後退
│   └── runbooks/<event>.md       # 障害時の手順
│
├── spec/                         # 機械が食う契約
│   ├── main.tsp
...
`operations/`のようなディレクトリを作らない。そこへ集まる内容——SLOの目標値、リリースと後退の手順、バックアップの方針、障害時の手順——は互いに無関係な種類であり、同じ役割の人が読むという一点だけで束ねられている。**読み手でディレクトリを切ると、同じ種類の文書が読み手ごとに分かれる。** SLOの目標値は運用者も実装者も読むし、容量の前提と同じ平面にあるべき現在状態の記述でもある。

章は複数の場所を指せる。ディレクトリは一か所にしか置けない。だから**読み手による分類は章立てで、置き場所は種類で**決める。行き先は§5.9の判定がそのまま決める。

| 内容 | それが変わると振る舞いか境界が変わるか | 置き場所 |
|---|---|---|
| SLOの目標値、測定上の定義 | 変わる。守ると約束した水準である | 正規文書（`docs/`直下） |
| バックアップのRPO/RTO、復旧の順序 | 変わる。失ってよい範囲の宣言である | 正規文書（`docs/`直下） |
| リリースと後退の手順 | 変わらない。やり方の話である | `docs/development/` |
| 障害時の手順 | 変わらない | `docs/runbooks/<event>.md` |

`backup-and-recovery`のような名前が二か所に分かれるのは分割ではない。**もともと別の種類が一つのファイル名に同居していた**ということである。目標値は宣言であり、訓練の手順はrunbookである。

**runbookを、それが操作するインフラ資材の隣へ置かない。** 資材の隣は変更する人にとって近いが、**障害の最中に探す人にとっては遠い。** 当番担当者は「どのディレクトリの資材の話か」を知らない状態で呼び出される。これは手順一般の規則ではない。あるコンポーネントを動かすためのREADMEは、そのコンポーネントの隣でよい。runbookが違うのは、**読み始める時点で原因が分かっていない**ことが前提だからである。
...
**配置:** `docs/development/release-and-rollback.md`。手順であって宣言ではない。
...
**配置:** 目標値（RPO / RTO）と復旧の順序は正規文書として`docs/`直下へ、実際に手を動かす手順は`docs/runbooks/`へ置く。
