...
機構そのものの働きがコードから復元できないときだけ書く。

判定は「この機構が壊れたとき、コードだけを読んで正しい直し方が分かるか」である。分かるなら書かない。

**何件のコンテキストに必要かは、領域の性質であって割り当てではない。** 表に対するCRUDの形をしたコンテキストには、説明すべき機構はたいてい無い。一方、フェイルクローズの拒否、鍵の寿命、リース、エポック、同一トランザクションでの捕捉といったものの上に建っているコンテキストにはたいてい在り、そういうものばかりでできた製品ではほぼ全部に在る。**想定した件数に合わせるためにファイルを消さない。** 規則は判定であって、件数は判定した結果でしかない。

この節が防ごうとしている失敗は、薄いファイルが並ぶことではない。**コードから読み取れないものの直し方を次の人へ伝えたはずの数段落が、件数を減らすために消されることである。**
