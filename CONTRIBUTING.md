...
- **応答が効果を含意しない段では、応答と、実際に起きたことの両方を確かめる。** 拒否がその代表で、ステータスだけを見るテストは「拒否」と応答してから操作を実行してしまう実装に対しても同じように通ります。書き込み、発行、失効を伴う成功経路も同じで、それが止まってもテストが通るなら、その段は確かめられていません。理由は [仕様先行の開発ワークフロー](docs/development/specification-first-workflow.md#when-the-response-does-not-entail-the-effect) にあります。
